...
The target project still needs Radical Pipelines conventions, typically in `AGENTS.md`, a CLI-specific `.pi/rp.md`, a dedicated conventions skill, or an external/local-only conventions location for not-owned repositories. Those conventions should define repository ownership and persistence policy, task lookup, pipeline slug format, pipeline artifact folders (for example `.pipelines/<pipeline-slug>` in owned repositories, or an external/local-only path for not-owned repositories), `/worktree` setup, branch naming, team spawning, commit format, fork/open-PR ownership policy, and Pi agent setup. During Pi setup, if no required agents are found in repository-local `.pi/agents/` or user-local/global `~/.pi/agent/agents/`, the workflow asks which Radical Pipelines agents to copy/paste and install before it launches teams. Reviewer agents write inspectable `spec-review-N.md`, `design-doc-review-N.md`, `plan-review-N.md`, `code-review-N.md`, and `docs-review-N.md` artifacts into the task's artifact folder.

Repository ownership is collected once during setup and stored in conventions for future runs. For not-owned repositories, all work and commits must happen in the configured fork; the orchestrator verifies that fork before launching agents and owns later open-PR behavior. No Radical Pipelines conventions may be added to the upstream project repository, `.gitignore` must not be modified there for Radical Pipelines, local-only ignores should use `.git/info/exclude`, and versioned/multi-user artifacts must live in a separate artifacts repository.
