...
Use imperative mood, sentence case, no period at the end.

A commit that introduces host-project product (code or external documentation) carries no agent-name provenance tag. A commit that changes only pipeline artifacts carries the name of the agent in parenthesis, or `assisted` if the commit was made in assisted mode.

Examples:

- Artifact-only commit: `Add intent (orchestrator)`, `Add spec (spec-reviewer)`, `Fix typo in plan (assisted)`
- Product commit: `Support for X`, `Fix bug Y`
