...
This file holds the conventions for this project: a shared section that applies regardless of the agentic coding tool, plus a Claude Code section. Read both at the start of any workflow.
...
`<branch-base>` is `<issue-number>-<short-description>`, where `<issue-number>` is the **GitHub** issue number (never the Linear id) and `<short-description>` is lowercase kebab-case. It must not contain `_`. The skill's branch grammar appends every other segment.
...
`artifacts-in-repo` — `.rp.md`, the artifact folder, and the worktree-root `.gitignore` entry live in this repository.
...
Worktrees live under `.worktrees/`. The orchestrator creates and removes them with raw `git worktree` shell commands — one worktree per branch, at `.worktrees/<branch>`. Agents never run `git worktree`; each occupies the worktree named in its Conventions block.
...
The orchestrator starts the loop itself. Cancel the loop on run close-out and after any owner-requested interruption.
