...
- **Push at run close-out** — push the pipeline branch to the remote so the artifacts are visible without entering the worktree. This happens after the final commit of the run has landed locally and applies to every outcome: normal close-out, blockers, owner-cancelled runs, and failures.
