...
Resolution is **per key**: the model and each named setting resolve independently, each by most-specific-wins — the agent entry's value, else the `Default`'s value, else today's behavior. Values are Claude Code value forms (a bare alias or first-party ID), passed to the spawn verbatim.
...
1. Treat **the model that just failed authentication** as unavailable for this retry (and the failed provider, unless the owner explicitly asked to use it).
...
4. Choose a provider-qualified replacement model from authenticated providers, **excluding the model that just failed authentication**. This recovery choice is a **fallback preference**, distinct from the per-agent **Agent models** configuration (which governs the initial spawn): prefer the owner's **environment/Pi default** authenticated provider/model when present; otherwise the closest suitable authenticated model for coding work. **Do not re-select the per-agent configured model that just failed.**
5. Retry the spawn with the explicit `provider/model` value. Reusing the same teammate name is acceptable when `pi-teams` replaces or kills the failed teammate automatically; otherwise remove or rename the failed teammate before retrying — this replacement model applies only to this recovery re-spawn, not to the agent's configured model.
...
Resolution is **per key**: the model and each named setting resolve independently, each by most-specific-wins — the agent entry's value, else the `Default`'s value, else today's behavior. Values are Pi provider-qualified `provider/model` forms, passed to the spawn verbatim. Pi exposes no confirmed per-teammate settings knob, so Pi entries are model-only.
