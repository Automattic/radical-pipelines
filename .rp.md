...
This file holds this project's conventions and its guardrails. The conventions apply to every agentic coding tool used here; the guardrails are this project's mandatory verification gates. Read this file at the start of any workflow. Per-tool conventions specific to Claude Code and Pi live in their own reference files and are loaded conditionally for the active tool.
...
A guardrail is an exact command judged by exit code, mandatory within the phase(s) it applies to, and tool-agnostic. The full definition and how to load the guardrails applicable to a phase live in `load.md`.
