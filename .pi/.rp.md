...
The orchestrator starts the loop itself. Pi has no `/compact`; token-limit recovery relies on Pi's built-in automatic compaction and falls through to restarting the affected agent on a fresh context.
