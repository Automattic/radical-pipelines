...
- **Closure action** — an action the owner invokes on a pipeline — opening its PR, merging its PR (both performed by the orchestrator), closing it without merging — bracketed by `before-`/`after-` hooks; beyond those steps, the work lives in the hooks or with the owner.
