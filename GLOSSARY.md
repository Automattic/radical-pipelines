...
- **Batch** — the set of build/document tasks dispatched since the previous review; scopes the reviewer's expected new work, never the review's boundaries (the reviewed diff spans the pipeline's whole work on the run; issues may attach to any task in the plan).
