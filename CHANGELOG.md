...
- [#311](https://github.com/Automattic/radical-pipelines/pull/311) [`867ed7e`](https://github.com/Automattic/radical-pipelines/commit/867ed7e3825b18d138f8ede6d9e61e37c238629c) Thanks [@luisherranz](https://github.com/luisherranz)! - BREAKING: name the pipeline's identifier the pipeline slug — `<pipeline slug>` throughout the skill — and derive the pipeline branch from it through the new optional `Pipeline branch format` convention (default the slug itself), with lane and merged-pipeline branches extending the pipeline branch; conventions format 3 renames `Branch naming` to `Pipeline slug`; `rp check` rejects a pipeline folder whose name is not a valid git ref
...
- [#301](https://github.com/Automattic/radical-pipelines/pull/301) [`5c1b8b3`](https://github.com/Automattic/radical-pipelines/commit/5c1b8b3b5091fd10fe095508f6f4e504abab541f) Thanks [@luisherranz](https://github.com/luisherranz)! - Resume a run after context compaction. The skill now opens with the instruction for a session whose context was compacted: treat the summary as unreliable, follow the skill again as at first sight, and resume a run under way without triage, checking which agents are already working before dispatching. Claude Code re-injects invoked skills after compaction on its own; the opencode plugin now does the same, re-supplying the skill body to a session whose activation left the active context, until it activates the skill again.

- [#312](https://github.com/Automattic/radical-pipelines/pull/312) [`fd81f2c`](https://github.com/Automattic/radical-pipelines/commit/fd81f2c0621aa0795e5181bf2005385c2ca04701) Thanks [@luisherranz](https://github.com/luisherranz)! - BREAKING: scope `rp_status` to one pipeline (`pipeline_slug`), one session (`session`), or every RP session (neither; the two exclude each other), reading the server only for the sessions in scope, narrowing `recentErrors` under a scope to the sessions RP recognizes in it plus the errors naming no session, and reading a transcript for `lastText` only in a session scope; drop `recentLoopTicks` and `skillActivations` from its payload and `updated` and `turns` from its rows; list each loop's `recentTicks` in `rp_loop_list`; rename `rp_spawn`'s `run` to `pipeline_slug`, validated as one segment, a valid git ref, without `_`, and the ledger row's `run` to `pipelineSlug`

- [#304](https://github.com/Automattic/radical-pipelines/pull/304) [`b23e78d`](https://github.com/Automattic/radical-pipelines/commit/b23e78d6f22b26449483745c04c5a8a3dbdd08f8) Thanks [@luisherranz](https://github.com/luisherranz)! - BREAKING: the run configuration — workflow, target phase, lanes — is recorded in the pipeline's `run-config.md`, which `rp check` reads; `--lanes`, `--target-phase`, `rp fingerprint`, and `--set lane=` are removed, `rp stamp` derives a file's lane from its path, and the Agents convention becomes free-form defaults.

- [#314](https://github.com/Automattic/radical-pipelines/pull/314) [`e664680`](https://github.com/Automattic/radical-pipelines/commit/e664680a644a7d5ee6a0ec20667f2755f2d373d2) Thanks [@luisherranz](https://github.com/luisherranz)! - BREAKING: Use constraints for binding owner rulings and proposals for open investigation, both in the intent and as individual phase-0 files with Target and Origin. Replace correction files with proposal files and intent decision entries with constraint files. Later input follows phase order and resolves independently on each target through its pins and a current approving wave. A constraint answers an owner claim by citing it and targeting the root artifact of the phase that raised it, without changing the intent or invalidating earlier phases. Convergence materials now expose challenges instead of corrections.
  
  Show every orchestrator synthesis of incoming work to the owner before recording it, including review and CI input. Explain the exact text, what is binding or open to investigation, and what it affects. Copy an already formatted issue directly only when it has no comments, references, links, or attachments to synthesize.
...
- [#316](https://github.com/Automattic/radical-pipelines/pull/316) [`0e23d48`](https://github.com/Automattic/radical-pipelines/commit/0e23d487a4b21481a7279fdcff9ff91ed7fe817e) Thanks [@luisherranz](https://github.com/luisherranz)! - Split the pull-request CI into four named checks

- [#305](https://github.com/Automattic/radical-pipelines/pull/305) [`ca07ab2`](https://github.com/Automattic/radical-pipelines/commit/ca07ab226706b026fda172ba4a9aec6fa64b4010) Thanks [@luisherranz](https://github.com/luisherranz)! - Record one decision per question answered, and state what the words answer so the entry reads without the exchange it came from. An answer to several questions bundled under one id cannot be cited beside the question it answers, and a quote followed only by the run it belongs to can satisfy the rule while leaving the decision undecodable.

- [#313](https://github.com/Automattic/radical-pipelines/pull/313) [`ab47087`](https://github.com/Automattic/radical-pipelines/commit/ab47087c8dba592680310f26956593af151953b6) Thanks [@luisherranz](https://github.com/luisherranz)! - A decision entry is shown to the owner as it will read and appended on approval, as every other intent item already is. An entry written from the orchestrator's own reading of an answer can state a ruling the owner never gave, and every later record and claim then cites it as the owner's.

- [#306](https://github.com/Automattic/radical-pipelines/pull/306) [`01bcd39`](https://github.com/Automattic/radical-pipelines/commit/01bcd39838be61b98625bfbca031c67fd3dc1f89) Thanks [@luisherranz](https://github.com/luisherranz)! - Record a decision as the ruling, approved by the owner, with its origin, instead of the owner's words quoted verbatim; append it only when it changes what the work must satisfy. Every other intent item is already the orchestrator's synthesis written on approval; verbatim quoting was a second guard on one section only, and an answer given as an option label put a token in the permanent record that means nothing without the question it answered. The origin keeps the trail to where the ruling was given, which the quote only carried indirectly. A ruling that leaves the work as it stands authorises nothing and no record cites it.

- [#313](https://github.com/Automattic/radical-pipelines/pull/313) [`3eeccf9`](https://github.com/Automattic/radical-pipelines/commit/3eeccf9e7f742c2281029116cfd5e9303de07744) Thanks [@luisherranz](https://github.com/luisherranz)! - An owner escalation stops the run through close-out, which cancels health monitoring and terminates the run's agents, instead of pausing the pipeline in place with the health loop still ticking; the owner's answer continues the pipeline through triage, as close-out and triage already described.

- [#313](https://github.com/Automattic/radical-pipelines/pull/313) [`58363a3`](https://github.com/Automattic/radical-pipelines/commit/58363a3e2d7f42e1953779227d0c4776639cce01) Thanks [@luisherranz](https://github.com/luisherranz)! - The health tick prompt names the skill file under this skill's folder. The prompt reaches a session seated in the run worktree, where a skill-relative path resolved against the worktree and the tick could not find its section.

- [#297](https://github.com/Automattic/radical-pipelines/pull/297) [`a9dfa21`](https://github.com/Automattic/radical-pipelines/commit/a9dfa21d4bbc4003e281a72af118d9c2bb9aa636) Thanks [@luisherranz](https://github.com/luisherranz)! - Assign intent item ids in `intent.md` only: an issue's bullets carry no ids, and synthesizing from an issue that already follows the format copies the body verbatim, adding the Origin lines and the ids without owner approval. Ids are the pipeline's addressing mechanism; the issue is the owner's request, and numbering its bullets is mechanical work that belongs to the pipeline, not a change for the owner to re-approve.

- [#313](https://github.com/Automattic/radical-pipelines/pull/313) [`c63a9b5`](https://github.com/Automattic/radical-pipelines/commit/c63a9b509bd1926f4124f63c04eb4c0d3dbd118d) Thanks [@luisherranz](https://github.com/luisherranz)! - The opencode tool file states that the plugin's `rp_` tools are direct tools of the orchestrator's session.

- [#309](https://github.com/Automattic/radical-pipelines/pull/309) [`288580b`](https://github.com/Automattic/radical-pipelines/commit/288580b20e3e61df943164575d176303876cd72e) Thanks [@luisherranz](https://github.com/luisherranz)! - Send a permission reply under the key the route's body names, so adjudicating a pending request works again: every reply was rejected as malformed, leaving the asking agent blocked until the request expired and its turn ended interrupted — both through the reply tool and through the automatic redirect of a read that resolves inside the agent's own worktree. The integration suite now adjudicates real pending requests against the running server, which is the only place a request body can be told apart from the one the server accepts.
...
- [#313](https://github.com/Automattic/radical-pipelines/pull/313) [`fad2e88`](https://github.com/Automattic/radical-pipelines/commit/fad2e8876af6ee03f1c6a42513476c9c738159d9) Thanks [@luisherranz](https://github.com/luisherranz)! - After a context compaction, following the skill again as at first sight includes reading anew every file it leads to. A session that took the summary's account of what it had read went on from the summary alone: it guessed the skill's location and the script's options instead of loading the conventions and the workflow.
