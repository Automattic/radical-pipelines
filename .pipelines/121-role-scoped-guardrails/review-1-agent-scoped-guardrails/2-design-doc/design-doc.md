...
The design rests on the current shape of the six files in this worktree, on branch `worktree-121-role-scoped-guardrails`. That branch **carries the base-run changes** (the base run, PR #124, is open and unmerged; `trunk` does not contain it), so the worktree files are the base-run state — the correct baseline for this review. `git diff --stat trunk...HEAD` confirms the base run touched four non-`.pipelines` files (`code-reviewer.md`, `code-writer.md`, `load.md`, `setup.md`) plus the changeset. Note the layout: conventions live under `skills/radical-pipelines/reference/conventions/`, but the agent files live at the repo root under `agents/`.
...
Guardrail touchpoints (verified): step 1.2 read (`:13`, "the code-tagged guardrails you must run before completing"); the step-3 "not a guardrail" disclaimer (`:36`); step 5 "Run the code-phase guardrails" (`:44-55`); the Guidelines blocker bullet (`:73`). Step 5 carries the writer obligations ("run **every** code-phase guardrail, exactly as its command is written … must pass before you commit, no bypass") plus the two-question outcome model (empty ⇒ run none; declared-but-unrunnable ⇒ blocker; runs-and-exits-non-zero ⇒ work). The file's selection is keyed to **phase** ("code-phase"), never to level — the base run narrowed the *reviewer*, leaving the writer phrased by phase.

### 3.4 `code-reviewer.md` — base-run state (the doc-reviewer template)

A fresh `code-reviewer` is spawned **once per batch** (`:8`); each instance is fresh and stateless. The base run restructured this file: step 1 gains a guardrail-read item (`:18`, "Read the guardrails applicable to the code phase — the code-tagged guardrails you must run during review"); step 2 still carries a guardrail bullet (`:32`, "No regressions / verification gates pass — run every guardrail …"); step 3 is behavior verification (`:34-36`); the `## Checks` template is `Check | Command | Result` (`:60-64`); the Guidelines carry "Run the guardrails." (`:97`) and the two-question outcome-model bullet (`:98-101`).

Note: the worktree `code-reviewer.md` still places the guardrail run inside the step-2 checklist bullet (`:32`) rather than in its own numbered step. The base run's design intended a dedicated step, but the realized file keeps it as a checklist bullet. For this review, **the template the doc-reviewer mirrors is the realized step structure plus the fail-fast/skipped/approval semantics the base design specifies** — and the doc-reviewer restructure (§5, D5) promotes guardrails to their own step as R7 demands. Where the realized `code-reviewer.md` and the R7 target structure diverge, R7 governs the doc-reviewer; the code-reviewer's own re-key (D4) is minimal and does not depend on which structure it ends in.
...
The writer's obligations are **unchanged in form**; only the set they range over re-keys from "code-phase" to "the gates that name `code-writer` or name no agents."

1. **Step 1.2 read (`:13`)** gains the selection phrase: read the guardrails that name `code-writer` or name no agents — the gates you must run before completing.
2. **Step 5 (`:44-55`)** is retitled "Run the writer guardrail selection" and reuses the defined term; the obligations survive verbatim in form ("run **every** gate, exactly as its command is written, all must pass before you commit, no bypass"), scoped to the selection and labeled the writer archetype. The empty-selection sub-case (`:52`), the drift-guard (`:53`), and exit-non-zero (`:54`) sub-cases re-word "code-phase guardrails" to the agent-name selection but are substantively unchanged.
3. **Guidelines blocker bullet (`:73`)**: re-word "a declared code-phase guardrail" to "a gate of your selection" — substance identical (a gate naming only other agents is never in the writer's selection, so the writer never attempts it and the drift guard can only fire on a selection gate), but once the file speaks in selections, keeping the broader phase term would leave a stale phrase against the term-reuse rule.

### D4 — `code-reviewer.md` re-keys its selection to agent name (reviewer archetype)

_Satisfies AC8._

The reviewer's behavior is **unchanged in form**; the selection re-keys from "code-phase" to "the gates that name `code-reviewer` or name no agents." The base run already gave this file the reviewer structure (guardrail-read in step 1, the judgment-checks-before-guardrails ordering, fail-fast permission, skipped recording, approving-iteration guarantee, the `pass`/`fail`/`skipped` Checks vocabulary). This review touches only the selection phrasing, labeled the reviewer archetype:

1. **Step 1 read (`:18`)**: "Read the guardrails applicable to the code phase" → "Read the guardrails that name `code-reviewer` or name no agents — the gates you must run during review."
2. **Step 2 guardrail bullet (`:32`)** and the **Guidelines bullets (`:97-102`)**: re-word every "guardrail applicable to the code phase" / "code-phase guardrails" occurrence to the agent-name selection. The fail-fast permission, the skipped recording, the approving-iteration guarantee, the two-question outcome model, and the drift-guard blocker all stand as written.

This re-key does not depend on whether the realized file keeps the guardrail run as a step-2 checklist bullet or promotes it to its own step (§3.4). It is a vocabulary re-key only; the reviewer structure is the base run's, preserved in form.
...
**`doc-reviewer.md` (fresh restructure, reviewer archetype, R7).** This file is in its pre-base 5-step shape and must be **freshly restructured** to the reviewer archetype, templated on the base run's `code-reviewer.md` and adapted for the docs vocabulary. The restructure mirrors what the base run did to `code-reviewer.md`: promote guardrails out of the mid-review bullet into their own step after the judgment checks, with fail-fast permission, skipped recording, the approving-iteration guarantee, a stateless line, and the `pass`/`fail`/`skipped` Checks vocabulary including the absent-vs-skipped distinction.
...
3. **`agents/code-writer.md`** (D3):
   - Step 1.2 read and step 5 (retitled "Run the writer guardrail selection") re-keyed to "name `code-writer` or name no agents," labeled the writer archetype; obligations unchanged in form; blocker bullet (`:73`) → "a gate of your selection."

4. **`agents/code-reviewer.md`** (D4):
   - Step 1 read (`:18`), step 2 guardrail bullet (`:32`), and Guidelines bullets (`:97-102`) re-keyed to "name `code-reviewer` or name no agents," labeled the reviewer archetype; fail-fast, skipped, approving-iteration, and drift-guard prose stand.
