...
13. No exit-code framing ("exit 0", "exit code", "exits non-zero", "judged pass/fail by exit code") remains anywhere guardrails are defined, captured, passed, run, or reviewed — across the guardrails reference, the convention loader, the setup convention file, the passing convention file (which carries the spawn-time Guardrails block), the assisted plan phase, the two code writers, the doc writer, and the two reviewers. Where the underlying meaning is load-bearing — notably setup's acceptance of a command that runs even though its check currently fails, and the reviewer/writer "the command ran but the check it describes isn't satisfied" outcome — the meaning is preserved while the exit-code words are removed.
