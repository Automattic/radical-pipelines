...
- **Changes:**
  ...
  - **Placement (precedent-matched, fence-relative):** put the new statement **inside** the fenced `.rp.md` canonical block (the ` ```markdown ` fence opening at line 9 and closing at line 44, where line 44 is the file's last line), in the `## Worktrees` section under the existing `## Worktrees` heading (the worktree-root the field references is the worktree this section already defines). This matches the `health-monitoring.md:13` defer precedent exactly: the per-tool command that completes that generic deferral — the `/loop 5m <prompt>` line in this file's `## Health monitoring` section — sits **inside** this same fenced block (lines 35-43), as does its orchestrator-only operational note ("The orchestrator starts the loop itself; the owner is not asked to run the command"). The design invoked that precedent, so the worktree-root derivation completing this change's generic deferral is placed the same way: inside the fence, with the existing tool convention it extends. Do not create any out-of-fence section — `claude-code.md` has no prose outside the fence, and this change introduces none.
- **Depends on:** Task 1
- **Traces to:** Spec Requirement 6 (outcomes hold under each supported tool); Requirement 4; design component "`reference/conventions/claude-code.md` (MODIFIED)" and decision "Generic instruction in `passing.md`, tool-specific derivation deferred" (which cites the `health-monitoring.md:13` precedent this placement matches).
- **Acceptance:**
  ...
  - The new statement appears **inside** the fenced `.rp.md` canonical block (between the opening ` ```markdown ` fence and the closing ` ``` `), within the `## Worktrees` section — matching where the `## Health monitoring` `/loop` command (the existing instance of the same `health-monitoring.md:13` defer precedent) sits in this file. No new out-of-fence section is added.
  - The statement is phrased minimally in the file's existing voice.
...
- **Changes:**
  ...
  - **Placement (precedent-matched, fence-relative):** put the new statement **inside** the fenced canonical `.rp.md` block for Pi (the ` ```markdown ` fence opening at line 7 and closing at line 41), in the `## Pi worktrees` section under its existing heading (which already defines the Pi worktree the field references, and already carries the `cwd` convention this derivation reuses: "Spawned teammates must use the Pi worktree as `cwd`, never the main checkout"). This matches the `health-monitoring.md:13` defer precedent exactly: the per-tool command that completes that generic deferral — the `/loop 5m <prompt>` line in this file's `## Health monitoring` section — sits **inside** this same fenced block (lines 32-40). Place this change's derivation the same way: inside the fence, with the Pi worktree convention it extends. Do not place it in the out-of-fence `## Setup actions` section (lines 43-65) — that section is unrelated agent-installation guidance, not the Pi worktree convention; and do not create a new out-of-fence section.
- **Depends on:** Task 1
- **Traces to:** Spec Requirement 6; Requirement 4; design component "`reference/conventions/pi.md` (MODIFIED)," decision "Generic instruction in `passing.md`, tool-specific derivation deferred" (which cites the `health-monitoring.md:13` precedent this placement matches), and the design's Risk/Open-Question "Pi worktree-root derivation is unverified against live tooling."
- **Acceptance:**
  ...
  - The new statement appears **inside** the fenced canonical `.rp.md` block (between the opening ` ```markdown ` fence and the closing ` ``` `), within the `## Pi worktrees` section — matching where the `## Health monitoring` `/loop` command (the existing instance of the same `health-monitoring.md:13` defer precedent) sits in this file — and not in the out-of-fence `## Setup actions` section.
  - The statement is phrased minimally in the file's existing voice.
...
- **Changes:**
  ...
  - Confirm `passing.md` stays tool-mention-free and that the derivation is deferred to `claude-code.md` / `pi.md` exactly as `health-monitoring.md:13` defers the loop command — including that each per-tool derivation sits **inside** that tool's fenced `.rp.md` canonical block, in the worktree section (`## Worktrees` for `claude-code.md`, `## Pi worktrees` for `pi.md`), matching where each file's `## Health monitoring` `/loop` command sits.
