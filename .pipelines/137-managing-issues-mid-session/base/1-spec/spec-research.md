...
### Q4 — Adversarial completeness/consistency check of the consolidated requirements

**A4 (spec-researcher), summarized; the requirements below were revised per these findings:**

1. **Contradiction found (the substantive one): R4/AC3 over-promised a universal return.** The original R4 said the orchestrator "returns to whatever it was doing … rather than being funneled toward starting fresh pipeline work." But the ONLY concrete current mid-session caller, `review-pipeline.md:12` (merged-pipeline change), is precisely a case where there is nothing to return to — the review is abandoned and proceeding to fresh pipeline work (what `manage-issues.md:53` already says) is correct. Rewriting Close out to "return to prior work" would BREAK that path; leaving it forward-only fails R4 for other callers. **Resolution:** R4 must require that `manage-issues.md` not hard-code ANY single next step (neither forward-only nor universal-return); control returns to the invoking situation, which decides. Revised R4/AC3 accordingly.
2. **Completeness:** Option A coverage is complete — R1's "whenever the orchestrator creates or modifies an issue" covers every Part-3 silent spot at the decision moment without enumerating them. Two small leak risks: (a) R5/AC4 could be misread as licensing edits to line 12's *trigger* (Option B) — scoped them to the route + return behavior only; (b) AC1's "Issues-convention routing" phrase could be picked at as dragging in metadata — made AC1's object explicitly the issue create/modify op.
3. **Testability:** AC1/AC3/AC4 are behavioral. AC2 ("locatable as a single general rule") leaned structural — reframed to the anti-duplication *property* ("not implemented as duplicated special-case instructions"). AC5 (non-regression) and AC6 (conformance to CLAUDE.md writing rules) are allowed — neither asserts section/wording/ordering.
4. **R2 vs (R5+R4) duplication tension is real but answerable:** the general rule (the *obligation*), `review-pipeline.md:12` (a review-domain *recognition+routing decision* that relies on the rule for the "how"), and `manage-issues.md`'s mid-session adjustment (*callee behavior*) are three statements at three different altitudes, not one statement thrice — duplication only springs if two of them spell out the same thing (e.g. both restate the capture Q&A). Added an explicit no-restatement / reliance clause to R2, mirroring the skill's existing conventions-loaded-pointer pattern.

All five recommended edits applied below.
...
- **R2 — Stated as one general rule, not per-spot patches; existing references rely on it.** The guarantee in R1 is expressed once, at a general level, so it covers every mid-session situation (current and future). It is NOT a set of special-case restatements added to individual procedures (which `CLAUDE.md` forbids when a general rule already covers the case). Existing references to the Managing Issues workflow (e.g. `review-pipeline.md`'s merged-pipeline route) *rely on* the general rule rather than restating it, consistent with `CLAUDE.md`'s no-duplication-across-reading-paths rule. (This mirrors the skill's established pattern: the conventions-loaded pointer sits at the entry-point root and downstream files inherit it silently.)
...
- **R4 — `manage-issues.md` is safe to enter mid-session, with no hard-coded next step.** Its framing no longer assumes it is reached only at session start. Crucially, it must not commit the orchestrator to ANY single fixed next step — neither today's forward-only "advance the issue into a pipeline" (`manage-issues.md:52-54`) nor a universal "return to prior work." Once the issue exists, control returns to the situation that invoked the workflow, and that situation determines where the orchestrator goes next: a merged-pipeline caller (`review-pipeline.md:12`) correctly proceeds toward fresh pipeline work because its review was abandoned; a mid-run caller would instead resume its run. (The front-door framing at `manage-issues.md:3` and the forward-only Close out at `manage-issues.md:52-54` are the spots that currently hard-code a single new-session, forward-only path.)
- **R5 — The existing precedent's route and return stay correct under the general rule.** The one existing mid-session hand-off (`review-pipeline.md:12`, merged-pipeline change → new issue) keeps routing correctly and returning correctly under R1–R4 — it neither contradicts the general rule nor restates it as a redundant special case. This requirement governs the *route and its return behavior* only; the condition that fires it (recognizing a merged-pipeline change as new work) is a review-domain judgment that is unchanged and out of scope (it is not a new recognition trigger added by this fix).
...
- **AC1.** Following the skill's reading paths, an orchestrator that decides mid-session (including mid-pipeline) to create or modify an issue is routed into the Managing Issues workflow — the issue create/modify operation goes through the capture Q&A and the Issues convention — not left to author the issue ad hoc. This holds for entry points beyond the single merged-pipeline case.
...
- **AC3.** Reading `manage-issues.md` as if entered mid-session, it does not hard-code a single next step: nothing in it forces the orchestrator to start fresh pipeline work on the just-created/modified issue, and nothing forces a return either — control goes back to the invoking situation, which decides. (For the merged-pipeline caller, proceeding toward pipeline work remains correct; for a mid-run caller, resuming the run is possible.)
- **AC4.** `review-pipeline.md`'s merged-pipeline hand-off still routes correctly and returns correctly (its post-issue path stays valid), and reads consistently with the general rule (no contradiction, no redundant restatement). Its triggering condition is unchanged.
...
- `manage-issues.md`'s body (capture Q&A steps 1–5, modify→read-first branch, approval gate, "report the issue reference") is already situation-neutral and reusable as-is; only its front-door framing (`:3`) and Close out (`:52-54`) need the mid-session-aware adjustment (R4) so neither hard-codes a single next step.
- R4's "control returns to the invoking situation" is the requirement; HOW that return is expressed (e.g. the caller names the return point at the call site, the idiom A3 documented for `work-on-an-issue.md` round-trips, vs. `manage-issues.md` handing control back generically) is a phase-2 design decision. The spec must not over-promise a universal "return to prior work," because the only existing caller (`review-pipeline.md:12`, merged-pipeline) correctly does NOT return — its review is abandoned and proceeding to fresh pipeline work is right.
