...
- Given a pipeline whose latest run is `rev-1`, when an amend layers on it, then the run is `amend-2-<desc>`, and a subsequent revision is `rev-3-<desc>` — one shared counter, the prefix naming the kind.
