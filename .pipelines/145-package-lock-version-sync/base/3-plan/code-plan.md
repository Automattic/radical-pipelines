...
These flows exercise the spec's acceptance criteria and edge cases end to end. The code-writer-e2e automates them; the reviewer can manually re-drive them. Each flow operates on a temp-dir fixture (a throwaway repo root with `package.json`, `.claude-plugin/plugin.json`, and a canonical `package-lock.json`) unless it explicitly targets the real repository's files.

**How each script is driven against a fixture (load-bearing constraint).** The two scripts must be driven by different mechanisms, because they locate their files differently:

- `scripts/sync-version.mjs` resolves `repoRoot` from `options.repoRoot` (defaulting to `REPO_ROOT`, derived from the script file's own location), NOT from `process.cwd()`. Its CLI block calls `syncVersion()` with no options and accepts no arguments or env var, so the real CLI cannot be pointed at a fixture directory. Therefore the sync flows (1–4) drive the behavior through the supported function-level override `syncVersion({ repoRoot: <fixture> })` — exactly as every existing `scripts/test/sync-version.test.mjs` case does — not via a subprocess against a fixture cwd. (Do not introduce a new CLI repoRoot mechanism; that would be a design change not in the design doc.)
- `scripts/check-version-sync.mjs`'s `main()` reads its files from the current working directory (relative paths). Therefore the check flows (5–7) drive it as a real CLI subprocess via `spawnSync(process.execPath, [scriptPath], { cwd: <fixture> })`, the way `validate-changesets.test.mjs` exercises its CLI.

**Requirement 9 — no covering task (acknowledged, not overlooked).** Spec Requirement 9 / its acceptance criterion ("a release version bump produces a 'Version Packages' pull request → that PR includes the updated `package-lock.json` alongside `package.json`, `plugin.json`, and the changelog") is satisfied by the reused release path: the extended `sync-version.mjs` patches `package-lock.json` during `npm run release:version`, and the changesets release action commits all resulting working-tree changes into the bot PR (Design "Decision: Reuse the release path"). Per the design doc (Risks: "documented action behavior, not asserted by the YAML … worth a verification note in a later phase"), this is NOT in-tree assertable — the changesets action's commit behavior cannot be proven from the YAML or any fixture. It is therefore deliberately deferred to manual inspection of a produced bot PR and has no covering task or flow here (mirroring how Flow 8 annotates the Requirement 11 branch-protection caveat). It is recorded here so the reader can see it was considered, not forgotten.

### Flow 1: Release version step syncs the lockfile end to end

- **Steps:** Build a fixture where `package.json` is at a new version `N` (e.g. `0.5.0`), `.claude-plugin/plugin.json` and `package-lock.json` are at an older version `M` (e.g. `0.4.0`). Call `syncVersion({ repoRoot: <fixture> })` (the supported function-level override; the real CLI cannot target a fixture). Inspect `package-lock.json` afterward and the returned result.
- **Expected:** `package-lock.json`'s top-level `.version` and `.packages[""].version` both equal `N`. `.claude-plugin/plugin.json`'s `version` equals `N`. The call returns successfully and its `changed` array includes `"package-lock.json"` (and the `plugin.json` target).
...
- **Steps:** Build a fixture where all three files already record the same version. Call `syncVersion({ repoRoot: <fixture> })` once; capture `package-lock.json` bytes. Call it a second time; capture again.
- **Expected:** After both calls, `package-lock.json` is byte-identical to its pre-run content (no change at all on a no-op), and each call's `changed` array excludes `"package-lock.json"` (no changed targets).
...
- **Steps:** Build a fixture as in Flow 1. Call `syncVersion({ repoRoot: <fixture> })` in an environment where no package registry is reachable (the sync uses only `node:fs`/`node:path`/`node:url` and performs no network I/O, so it must succeed regardless of registry reachability).
- **Expected:** The call completes successfully and `package-lock.json`'s two version fields are updated to `N`. No network access is attempted by the lockfile version sync.
...
- **Depends on:** Task 1 (the live correction itself only needs the extended sync from Task 1), Task 2 (this task's last acceptance criterion runs `node scripts/check-version-sync.mjs`, created in Task 2)
...
- **Changes:**
  - Automate Flows 1–4 against `scripts/sync-version.mjs` (release-step sync, idempotency, two-fields-only-no-churn, offline success) by calling `syncVersion({ repoRoot: <fixture> })` against temp-dir fixtures that include a canonical `package-lock.json`. Do NOT drive these flows via a subprocess CLI against a fixture cwd: the sync CLI resolves `repoRoot` from the script file's own location, not `process.cwd()`, so it cannot be pointed at a fixture; the `syncVersion({ repoRoot })` override is the only supported mechanism (see the E2E test plan intro).
