...
- **Changes:** Replace the sentence `Context-window limits are handled by **each tool's own auto-compaction**, not by the monitor.` with `Context-window limits are handled by each tool's own mechanism, not by the monitor.` Change only this sentence; leave the rest of the Health-monitoring description and every other part of `setup.md` untouched by this task.
- **Depends on:** none
...
- **Depends on:** none (the row may be added before `opencode.md` exists; Task 3 creates the file it points at)
...
- **Files to change:**
  ...
  - `package.json` (root) — converted to a workspace root (see Task 7 for the workspace declaration; this task may add the `workspaces` field if Task 7 has not, but the root stays `private: true`).
- **Changes:**
...
- **Depends on:** Task 7 (workspace root must exist for the sub-package to resolve) — but the sub-package files themselves can be authored first; ensure the root `workspaces` field and the sub-package both exist before install/build is expected to work. (Sequence: author this sub-package, then complete Task 7's root/workspace edits; treat Task 7 as completing the wiring.)
...
- **Acceptance:**
  ...
  - The root `package.json` remains `private: true` (only the sub-package publishes) and declares the sub-package as a workspace member.
...
- **Goal:** Turn the flat repo into a workspace root that contains the new sub-package, and extend the version-sync and changeset tooling so the sub-package's version stays in lockstep with the root without becoming an independent changeset target.
- **Files to change:**
