...
3. **The opencode session DB is the agent ledger.** Each spawned session durably carries its agent, model, directory, and timestamps; RP adds the run-unique instance name by setting the session title to `rp:<run-branch>:<instance-name>`. A module-singleton in-memory map (instance name, run, spawner) serves live lookups; the title makes the ledger reconstructable from `GET /api/session` after any restart.
...
5. **The plugin module is imported once per daemon** (verified), while `setup(ctx)` re-runs per directory scope. All global concerns — the event listener that notifies each child's spawner on the child's first terminal event, and the loop timers — live behind module-level singleton guards.
...
- `rp_spawn {name, agent, model, directory, prompt, run}` — `agent` is the profile name (an RP agent id materialized by install, e.g. `spec-reviewer`); `name` is the run-unique instance name the orchestrator assigns per the generic skill's spawn step (multi-instance profiles are the norm, so the ledger keys by `name`, never by `agent`); `run` is the run branch. The tool validates `agent` against `ctx.agent.list()` (create does not validate — a typo would make a dead session); parses `model` from the convention's `provider/model[#variant]` string into `Model.Ref {providerID, id, variant?}` (the API rejects the string form); calls `ctx.session.create({agent, model, location:{directory}})`; records `sessionID → {name, run, spawner}` in the in-memory ledger map, where `spawner` is the caller's `toolCtx.sessionID` (unspoofable; opencode has no session parenting — the spawner exists only because rp_spawn records it); posts the initial prompt; returns the **session ID — the stable identifier** for all addressing. The durable title `rp:<run>:<name>` is asserted by the completion listener after the first turn (opencode auto-generates a title on the first turn, overwriting earlier renames; titles set after the first turn persist).
...
- `rp_loop_start {interval, prompt, target_session?}` (`target_session` optional, defaulting to the calling session — the orchestrator needs no self-ID discovery) → registry entry + guarded `setInterval`; returns the loop id. Per tick: `GET /api/session/active`; target running → skip the tick; idle → inject the prompt (queue). `rp_loop_list` reads the registry; `rp_loop_cancel {id}` clears the timer and deletes the entry.
...
**Completion notifications:** the singleton listener (module-guarded `ctx.event.subscribe`, global across projects and worktrees) watches terminal events (`session.execution.succeeded`/`.failed`) on ledger sessions and notifies the spawner — resolved from the ledger's `spawner` field — **on each child's first terminal event only**, injecting a completion message (queue delivery) into the spawner's session and re-asserting the child's durable title. Later turns do not notify: subsequent completions are communicated by the agents themselves via `rp_send` (the skill's existing completion-report protocol), and subsequent failures are monitor observables through the listener's error log and `rp_status`. Restart behavior is explicit: the in-memory map (including `spawner`) dies with the daemon and in-flight executions die with the process, so a post-restart terminal event on a pre-restart child resolves no spawner and produces no notification — consistent with the git-based resume model, where the resumed orchestrator re-spawns agents from committed state.
...
- **Choice:** no RP state file for agents. Live lookups: the module-singleton in-memory map `sessionID → {name, run, spawner}` written by `rp_spawn` (`spawner` = the caller's session, recorded because opencode has no session parenting). Durable key: session title `rp:<run>:<name>` — keyed by the run-unique instance name, not the profile name, since multi-instance profiles (two `spec-reviewer`s in a round) would collide — asserted after the first turn (auto-title fires only on turn 1); agent/model/directory/timestamps are already persisted by create and survive restarts. The title omits the spawner: durable reconstruction serves the monitor and a resumed orchestrator, and post-restart there is never a notification to route. The orchestrator addresses agents by session ID; a re-spawn yields a new ID that is explicitly propagated (per the health contract) — never silently re-bound.
...
### Decision: Completion notifications fire on the first terminal event only

- **Choice:** the listener notifies a child's spawner on the child's first terminal event (`succeeded` or `failed`) and never again. Later completions are reported by the agents themselves via `rp_send` — the protocol the generic skill already relies on ("agents message you when their work completes") — and later failures reach the monitor through the listener's error log and `rp_status`. All terminal events still feed the listener's internal records.
- **Alternatives:** notify on every terminal event (rejected: every `rp_send` into an idle child creates an execution and every execution ends with a terminal event — verified — so every lead ↔ researcher exchange would also wake the orchestrator with a full LLM execution, multiplying orchestrator turns and token cost during exactly the phases with the most inter-agent traffic, for exchanges the spawner is not a party to); notify only for the execution started by the spawn prompt (equivalent in practice, but requires execution-ID tracking for no added value).
- **Trade-offs:** orchestrator wake-ups stay proportional to spawns rather than to message traffic; the cost is that a later-turn failure reaches the orchestrator via the monitor's next tick instead of a push — the same latency class as on Claude Code, where mid-life agent failures are also monitor-detected.
- **Traces to:** Requirement 11 (notification when the agent completes); supports 14.
...
- **Auth and provider-call network errors:** structured errors on `session.execution.failed`/`session.step.failed` — auth is `type: "provider.auth"` with the provider's HTTP body verbatim in `message` (live-verified; ready for the escalation payload's error-verbatim field); non-auth types (live-verified sibling: `provider.no-route`) are treated as the transient class with retry-once semantics that do not depend on the exact type string. This channel covers provider-call failures — errors that terminate an execution.
- **Tool-call network errors:** a tool call failing inside an agent's turn ordinarily returns an error result to the model and the turn continues; nothing verified shows it reaching `session.execution.failed`. These failures are observed as on Claude Code — the agent's own report, worktree state, and messaging state, which the monitor reads each tick, with the stall signal as the backstop — and the retry-once recovery action is channel-independent. Whether opencode additionally surfaces them on an event channel is an open question (below).
...
- **Network-error surfacing.** Two parts: the transient-class structured-error `type` for provider-call failures was not reproduced live (auth and no-route siblings were), and whether a tool-call network failure inside a turn surfaces on any event channel at all. Verified by: a Build-phase integration check forcing a tool-call network failure (e.g. webfetch to an unroutable address) and recording what surfaces. Safe: provider-call failures use the live-verified channel, tool-call failures are observed via the monitor's existing signals (agent report, worktree and messaging state, stall backstop), and the recovery action is channel-independent.
