...
- Given an installed RP at an older release, when the owner follows the documented update procedure, then the RP-provided version surface reports the newer release and the updated skill and agents are the ones in effect.
...
- Given a directed inter-agent message that fails, errors out, or is never delivered, when the health monitor acts, then the delivery failure is observable, the message is re-sent, and on a second failure the target agent is restarted.
...
- Given a tool call failing with a transient network error, when the health monitor acts, then the failure is observable and the call is retried once, and on a second failure the monitor waits one interval and retries before escalating.
