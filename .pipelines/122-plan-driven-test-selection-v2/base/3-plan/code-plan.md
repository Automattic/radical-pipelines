...
**Important divergence A — load.md/setup.md enumeration (resolved against the actual post-#121 codebase, not a prior-phase decision):** Design §6 and spec R8/AC5 say the *gate-running-agents enumeration* lives in `load.md` (L30) and that `setup.md` has an *illustrative example row* (L189). Neither is true in this worktree. #121 (merged as PR #124) placed the gate-running-agents enumeration entirely in `setup.md`: line 183 (`one or more of \`code-writer\`, \`code-reviewer\`, \`doc-writer\`, \`doc-reviewer\``) and line 185 (the per-task/per-pipeline reminder, which also names those agents). `load.md` contains **no** such enumeration, and `setup.md` has **no** illustrative agents example row. The spec's *intent* — every live place that enumerates the gate-running agents must name the two new writers and the views must agree — is satisfied by editing the real locations (`setup.md` L183 and L185). `load.md` needs no edit. The "setup.md example row" sub-edit is vacuous (nothing to change) and is dropped. This is recorded so the reviewer and writers are not misled by the design's stale `load.md`/`L189` references. The `.changeset/agent-scoped-guardrails.md` enumeration is a historical record of #121 and is out of scope (editing it would be migration/historical churn, contrary to AC8).

**Important divergence B — writer guardrail vocabulary post-`a0e3fd9` (resolved against the actual codebase, not a prior-phase decision):** Design §3 describes the writer's guardrail handling using vocabulary that commit `a0e3fd9` ("Gate reviewer guardrails on judgment; drop selection vocabulary," an ancestor of HEAD here, landed after the #124 merge the design was grounded against) deliberately **removed** from `agents/code-writer.md` and every sibling agent file. Specifically, the design's references to a Gather-context *guardrail-read line* that "names the agent," to an agent-scoped *"guardrail selection,"* and to a *"two-question outcome model"* / *"Run the writer guardrail selection"* step are all stale. In the **actual** current `agents/code-writer.md`: (1) Gather context has **no** guardrail-read line — the writer learns its gates from the `## Conventions` block's `Guardrails:` field the orchestrator passes at spawn (autonomous-workflow.md L66: "the gates that name this agent"), not by reading a line that names itself; (2) the step is named **"Run the guardrails,"** and runs "every gate in the guardrails convention, exactly as its command is written" — no per-agent "selection"; (3) there is **no** "two-question outcome model" label — the step uses a plain three-bullet sort (no guardrails convention → proceed; a declared gate that cannot execute → blocker; a gate that runs and exits non-zero → work). The same `a0e3fd9` treatment applies to `doc-writer.md` and `code-reviewer.md`, so the two new writers must match this shape to stay consistent with the rest of the agent set. Tasks 3/4 and Flow 3 below are anchored on this real current shape; the new writers' guardrail step is modeled on the current "Run the guardrails" three-bullet sort, **extended** so the writer runs both the guardrails convention's gates and the required-test-commands floor (the floor outcomes sort the same way: a floor command that cannot run → blocker/drift, since the plan-reviewer already validated it runs; a floor command that runs and reports failures → work). No removed vocabulary ("guardrail selection," "two-question outcome model," a self-naming guardrail-read line) is re-introduced.
...
- **Steps:**
  ...
  3. Confirm the task-block template has a `- **Type:** tdd | e2e` line directly after `- **Goal:**` and before `- **Files to change:**` (i.e., `Type` sits between `Goal` and `Files`).
...
- **Steps:**
  ...
  5. Confirm each file has a "Run the guardrails" step (the current `code-writer.md`'s name) that runs BOTH the gates in the guardrails convention the orchestrator passes AND the required-test-commands floor before commit, sorting both via the current three-bullet model (no guardrails convention → proceed; a declared gate or floor command that cannot execute → blocker/drift, since the plan-reviewer already validated the floor runs; a gate or floor command that runs and reports failures → work). Confirm neither file uses the removed vocabulary `git grep -n "guardrail selection\|two-question\|two questions" agents/code-writer-tdd.md agents/code-writer-e2e.md` (expect no matches).
  6. Confirm neither file's Gather context contains a self-naming guardrail-read line (the writer learns its gates from the orchestrator-passed `Guardrails:` field, matching the current `code-writer.md`), and that the e2e writer's self-containment guideline additionally names the E2E test plan section as an input.
- **Expected:** `code-writer.md` absent; both new files present with correct frontmatter and the shapes above; no verification/derive step in either; both run the guardrails convention's gates + the floor before commit, sorted via the three-bullet model; no removed guardrail vocabulary and no self-naming guardrail-read line; the greps at steps 4 and 5 return no matches.
...
- **Changes:**
  ...
  - In the task-block template, add `- **Type:** tdd | e2e` directly after `- **Goal:** ...` and before `- **Files to change:** ...` (so `Type` sits between `Goal` and `Files`), per design §1.
...
- **Acceptance:**
  ...
  - The task-block template has `- **Type:** tdd | e2e` immediately after `- **Goal:**` and before `- **Files to change:**` (between `Goal` and `Files`).
...
- **Type:** tdd
- **Goal:** Create the new `code-writer-tdd` agent file — unit tests via TDD only, carrying the UI-conventions duty, running the guardrails convention's gates AND the required-test-commands floor before commit, with no behavior-verification or e2e-self-derivation step — per design §3 (corrected for the post-`a0e3fd9` guardrail vocabulary; see Overview divergence B).
- **Files to change:** `agents/code-writer-tdd.md` (new)
- **Changes:** Author a new agent file modeled on the **current** `agents/code-writer.md` scaffolding (role frame, gather-context, run-the-guardrails, commit-and-report, guidelines), collapsed to four steps — **Gather context / Implement with TDD / Run the guardrails / Commit and report** — with these specifics. Model every step's wording on the current `code-writer.md` as it exists in this worktree; do not re-introduce the vocabulary `a0e3fd9` removed (no "guardrail selection," no "two-question outcome model," no self-naming guardrail-read line — see Overview divergence B):
  - Frontmatter: `name: code-writer-tdd`; description naming unit tests via TDD.
  - Gather context: model on the current file's two-item Gather context (read the assigned task block; if the orchestrator cited a review file plus issues scoped to the task, read and address them) and add reading the Required test commands section of `code-plan.md` as an input. Do **not** add a guardrail-read line — the current file has none; the writer learns its gates from the `## Conventions` block's `Guardrails:` field the orchestrator passes at spawn.
  ...
  - Run the guardrails: keep the current step's name ("Run the guardrails") and its three-bullet sort, **extended** so the writer runs two command sets — the gates in the guardrails convention the orchestrator passes AND the required-test-commands floor from `code-plan.md` — before commit, sorting both with the current model: no guardrails convention → proceed (the floor still runs); a declared gate or floor command that cannot execute → blocker (drift; the plan-reviewer already validated the floor runs); a gate or floor command that runs and reports failures → work, not a blocker; no bypass; all pass before commit. Drop the current file's back-reference to the removed step-3/4 verification.
  - Commit and report: as in the current writer.
  - Guidelines: carry over the current writer's guidelines, adjusting the self-containment guideline to: "The task block plus the Required test commands section of `code-plan.md` are your inputs. You should not need the prompt, spec, design doc, or other tasks in the code plan."
  - The file must not contain a "Behavior verification" step or a "Derive end-to-end tests" step, nor the removed vocabulary "guardrail selection" / "two-question" / a self-naming guardrail-read line.
- **Depends on:** none
- **Traces to:** Spec R5, R6 / Acceptance criterion 3 / Design §3 (corrected per Overview divergence B)
- **Acceptance:**
  ...
  - It has a "Run the guardrails" step (the current file's name) that runs both the guardrails convention's gates and the required-test-commands floor before commit, sorted via the current three-bullet model (no guardrails convention → proceed; a declared gate or floor command that cannot execute → blocker/drift; a gate or floor command that runs and reports failures → work; no bypass; all pass before commit).
  - Its Gather context names the task block and the Required test commands section as inputs and contains no self-naming guardrail-read line.
  - The file does not contain the removed vocabulary "guardrail selection" or "two-question"/"two questions".
...
- **Type:** tdd
- **Goal:** Create the new `code-writer-e2e` agent file — implements the planner's e2e specs from `code-plan.md`, runs the guardrails convention's gates AND the required-test-commands floor before commit, with no behavior-verification or e2e-self-derivation step and no RED/GREEN/REFACTOR — per design §3 (corrected for the post-`a0e3fd9` guardrail vocabulary; see Overview divergence B).
- **Files to change:** `agents/code-writer-e2e.md` (new)
- **Changes:** Author a new agent file sharing the `code-writer-tdd` scaffolding verbatim (role frame, gather-context, run-the-guardrails, commit-and-report, most guidelines), collapsed to four steps — **Gather context / Implement the planned e2e flows / Run the guardrails / Commit and report** — with these specifics. As with Task 3, do not re-introduce removed vocabulary (no "guardrail selection," no "two-question outcome model," no self-naming guardrail-read line — see Overview divergence B):
  - Frontmatter: `name: code-writer-e2e`; description: implements the planner's e2e test specs from `code-plan.md`.
  - Gather context: model on `code-writer-tdd`'s Gather context (read the assigned task block — which names the flow(s) it implements; if the orchestrator cited a review file plus issues scoped to the task, read and address them) and name both the E2E test plan section AND the Required test commands section of `code-plan.md` as inputs. No guardrail-read line (same as `code-writer-tdd`).
  ...
  - Run the guardrails: identical to `code-writer-tdd`'s "Run the guardrails" step (the guardrails convention's gates AND the required-test-commands floor; the current three-bullet sort applied to both; no bypass; all pass before commit).
  - Commit and report: as in `code-writer-tdd`.
  - Guidelines: shared with `code-writer-tdd`, except the self-containment guideline carve-out names both shared sections: "The task block, the E2E test plan section, and the Required test commands section of `code-plan.md` are your inputs. You should not need the prompt, spec, design doc, or other tasks in the code plan."
  - The file must not contain a "Behavior verification" step or a "Derive end-to-end tests" self-derivation step, nor the removed vocabulary "guardrail selection" / "two-question" / a self-naming guardrail-read line.
...
- **Acceptance:**
  ...
  - It has a "Run the guardrails" step (the current file's name) that runs both the guardrails convention's gates and the required-test-commands floor before commit, sorted via the current three-bullet model.
  - The file does not contain the removed vocabulary "guardrail selection" or "two-question"/"two questions", and its Gather context has no self-naming guardrail-read line.
  - Its self-containment guideline names the task block, the E2E test plan section, and the Required test commands section as inputs.
