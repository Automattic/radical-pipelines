...
- Given a pull request in which the lockfile version fields disagree with `package.json`'s version, when the pull-request gate runs, then the drift guard fails with a message identifying the inconsistency.
- Given a pull request in which `package.json`'s version is hand-edited so it disagrees with `.claude-plugin/plugin.json` and the lockfile, when the pull-request gate runs, then the drift guard fails (the inconsistency is not allowed through, unlike with dependency-tree-only validation).
