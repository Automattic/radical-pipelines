...
Must this first Codex release provide health monitoring and detached recovery?

### Answer 9

No. Health monitoring and detached recovery are optional for this first release.

### Question 10

What does cross-tool workflow parity require when Codex uses different runtime mechanisms from Claude Code?

### Answer 10

Parity covers normal successful execution, committed artifacts, and phase outcomes. It does not require identical runtime machinery.
...
- Tool-dependent conventions are isolated from shared project conventions. Codex support must supply the required team-spawning conventions for Codex before the existing completeness gate permits a run. Sources: `skills/radical-pipelines/reference/conventions/load.md`, `skills/radical-pipelines/reference/conventions/claude-code.md`.
...
2. Every in-scope Codex surface must support autonomous and assisted workflows with parity for normal successful execution, committed artifacts, and phase outcomes.
...
5. Surface-specific implementations may differ while preserving the required workflow outcomes.
6. Codex support must not regress existing Claude Code workflows.
7. Health monitoring and detached recovery are optional for the first Codex release.
