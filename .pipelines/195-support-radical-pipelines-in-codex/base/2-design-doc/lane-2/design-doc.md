...
At invocation, Radical Pipelines identifies the active adapter and resolves a committed configuration view before creating pipeline state. The shared project file supplies tool-neutral conventions and may route the active adapter to a separate committed file. The adapter may provide a durable inline fallback for existing configurations. Only the active tool participates in resolution. Committed completeness is validated before local overrides are applied; any failure enters the existing setup path before branches, worktrees, or artifacts are created.
...
- **Tool adapters:** `conventions/claude-code.md` owns Claude Code discovery, fallback, spawn/model conventions, native prerequisites, and required monitoring. New `conventions/codex.md` owns the corresponding Codex conventions and declares monitoring optional.
...
- **Codex distribution/configuration:** Native Codex metadata binds the canonical skill and profiles to each local Codex surface. Concrete filenames and schema follow the accepted Codex discovery convention selected during implementation.
- **Shared autonomous and resume references:** Become mechanism-neutral where they describe persistent pairs, spawn ownership, lane scheduling, and monitor lifecycle. Each spawning owner receives every required spawn input.
...
5. Validate the committed merged view and native prerequisites.
...
The orchestrator reserves capacity for root, analyst, and researcher before starting a persistent pair. Additional isolated lanes run in waves when capacity is constrained. Lane isolation, branch ownership, and mutual blindness are preserved regardless of scheduling.
...
### Decision: Delegate the persistent researcher with complete spawn inputs

- **Choice:** Root spawns the analyst with a fully resolved researcher descriptor; the analyst owns exactly one researcher for its lane's complete Q&A.
- **Alternatives:** Require root to spawn both persistent roles; let the analyst rediscover project configuration; replace the pair with one agent.
- **Trade-offs:** Nested spawning requires three live instances and explicit input passing. It preserves analyst/researcher ownership while keeping shared phase language independent of runtime topology.
- **Traces to:** Requirement 2; acceptance criteria 2 and 7.

### Decision: Schedule isolated lanes in capacity-aware waves

- **Choice:** Reserve the root/analyst/researcher minimum, then run additional isolated lanes independently as capacity permits. Keep shared-worktree agents sequential.
- **Alternatives:** Require every lane to run simultaneously; serialize all work; relax lane isolation.
- **Trade-offs:** Waves may extend elapsed time, but preserve required roles, isolation, commits, and outcomes on constrained surfaces.
...
- Codex convention-based repository discovery and native local agent, messaging, model, shell, and worktree capabilities;
...
- **Configuration failure:** A missing/duplicate route, unreadable or out-of-repository file, conflicting unit, incomplete committed convention, or missing native prerequisite stops before pipeline mutation. The error identifies the adapter ID, path, failed check, exact missing items, and setup action. Local overrides cannot mask it.
...
- **Capacity shortage:** Work waits until root plus a persistent analyst/researcher pair can run. A required Codex surface that cannot sustain the nested pair fails release validation rather than silently narrowing roles or surface coverage.
...
| 8. Incomplete setup | Missing route, file, Team spawning, or native profile prerequisites each stop before new pipeline state and surface the setup path. |
...
Automated verification runs the existing test suite, changeset validation, version drift checks, and the selected native manifest validator. If the Codex manifest stores the project version, version synchronization and drift tests cover it. Manual records include surface/runtime version, transcripts, role/ID/model records, worktree and branch listings, logs, SHAs, artifacts read from commits, guardrail output, tracker state, pushes, and close-out results.
...
- **Nested capacity:** Desktop, CLI, and IDE must each support root, analyst, and the analyst's researcher concurrently. Validate this before release. Failure returns the design for revision rather than changing role ownership or excluding a required surface.
