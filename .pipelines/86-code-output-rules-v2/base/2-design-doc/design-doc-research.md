...
- **Research — precedent for prose-only Code phases in this self-hosting repo (researcher-confirmed, with first-hand cross-check this session):**
  ...
  - **`Type` is constrained to `tdd | e2e` by the current template + strict router; `tdd` is the right value for a prose task (matching `122`); the "omit Type" precedents are NOT clean precedents to rely on.** `134`/`137`/`123` (and older `90`, `91`, `95`, `121`) have `0` `Type:` lines in their plans — they used **custom, non-template plan formats** that omit the field. **Correction (researcher-verified firsthand, superseding an earlier mistaken claim in this record):** `134` (ran 2026-06-17) and `137` (2026-06-18) did **NOT** run under a pre-split unsplit `code-writer`. The single `code-writer.md` was deleted 2026-06-15 (`b7a5101 "Delete obsolete code-writer agent"`), after the tdd/e2e split on 06-13 (`9009152`), and strict tdd|e2e routing was already in the phase-4 reference by 06-13/15 (`07e51b1 "Dispatch by task type in phase-4 reference"`, `31bc91f`). So 134/137 ran *after* the split, *after* the file deletion, and *after* strict routing landed. Their product commits carry the `(code-writer)` label (e.g. `4c21de3`), but that is only the commit-message agent label they used — the `code-writer.md` agent file did not exist then, so the label does **not** prove a literal `code-writer` agent was dispatched. So: omit-`Type` plans were produced *post-routing*, but **how the orchestrator dispatched a Type-less, non-template plan is not determinable from the artifacts** (possibly assisted/human-driven; not a verified clean autonomous strict-router dispatch) — hence "omit Type" is not a precedent this design can rely on under the current `code-plan-writer.md` template. The decisive, supported facts: (1) the current template offers only `Type: tdd | e2e` (`code-plan-writer.md:54`) and the current router (`4 - code.md:3,35`) dispatches strictly by that value to two named writers — there is no third type or Type-less route in the template/router; (2) the ONE pipeline that used the `Type` field on pure prose — `122` — chose **`tdd`** for every task. So under the current template a prose task is typed `tdd`, and its contract is satisfied by inspection (123/134/137/90's universal mode), not by 122's banned structural test. (`145-package-lock-version-sync-v2` also uses the current `Type` field but is NOT prose — it ships real runtime code `scripts/check-version-sync.mjs` with genuine `spawnSync` behavioral tests; not a counterexample.)
...
- **Options considered:**
  ...
  4. **Omit the `Type` field and add a dedicated inspection-verification task (the 134/137/123 plan shape).** Rejected as the basis for our design: those plans used custom, non-template formats that omit `Type`, and the dispatch mechanism for a Type-less plan is **not verifiable from the artifacts** (see the corrected research bullet above — 134/137 ran post-split/post-routing, so their `(code-writer)`-labelled commits do not establish an unsplit dispatcher; how a Type-less plan was routed is unknown). The **current** `code-plan-writer.md` template offers only `Type: tdd | e2e` and the **current** router dispatches strictly by it (`4 - code.md:3,35`); writing a Type-less plan would diverge from the current template and rely on an unverified dispatch path. We keep these pipelines' *verification mode* (inspection/grep, no test — which IS well-attested) but follow the current template by giving the prose task a `Type`.
  5. **Type prose tasks `tdd`, verify by inspection at the gate (no test), AND additionally amend the writer profiles to add an explicit inspection-only branch to their test contract.** Verification path correct; profile amendment **rejected for THIS feature on owner scope decision.** Making the inspection affordance explicit in `code-writer-tdd.md`/`code-writer-e2e.md` is a *general* tool change — it governs every future prose/instruction-only code task, not the output rules that are #86's subject. #86 edits those writer profiles **only** for Rule 1 / Rule 2 (its actual subject). The amendment is deferred to a separate effort (see Open Questions). It is also unnecessary for feasibility: every prior prose-only pipeline (123/134/137/90) shipped with the writer profiles unchanged.
  6. **Chosen — decision-only: type prose tasks `tdd` (the only value the current template/router accepts for a non-e2e task, and 122's precedent on pure prose), and verify them at the gate by `code-reviewer` inspection with no test written (the universal mode, per 123/134/137/90), relying on the reviewer's already-authorized inspection mode plus the `AGENTS.md:17` ban — no tool/profile modification for dispatch or verification.** See Decision.

- **Decision (decision-only — no tool/profile modification for dispatch or verification):** **This feature's Code phase types its prose profile-edit tasks `tdd` and verifies them by `code-reviewer` inspection at the existing review gate with no test written** — the verification mode every prose-only pipeline in this repo has used (123/134/137/90), resting on the reviewer's already-authorized inspection mode (`code-reviewer.md` step 3 + line 45) and the `AGENTS.md:17` ban on structural prose-tests. This is a verification/dispatch **decision** for #86, not a change to the writers, the planner, or the routing. The only files #86 edits remain the output-rules surfaces (the producing/reviewing profiles for Rule 1 / Rule 2 per Topics 1, 4, 7); the writer/planner profiles are **not** modified for dispatch or verification (see Dropped, below). Concretely:
  1. **Dispatch (planner side).** The `code-plan-writer` uses the existing `Type: tdd | e2e` field unchanged, and #86's prose profile-edit tasks are typed **`tdd`** — the only non-e2e value the current template (`code-plan-writer.md:54`) and strict router (`4 - code.md:3,35`) accept, and the choice `122` made for its own pure-prose tasks. Each task's Acceptance is written as **observable, inspectable properties of the shipped profile prose** (e.g. "`code-writer-tdd.md` carries the canonical Rule 2 statement and the narrower `code-writer-tdd.md:33` line no longer exists"), NOT as runtime behavior. No `code-plan-writer.md` edit, no new `Type` value, no third writer — the planner and routing are untouched.
  2. **Execution (writer side) — no profile change.** The dispatched `code-writer-tdd` makes the prose edits and runs the guardrail gates unchanged. It does **not** write a content-assertion test (forbidden by `AGENTS.md:17`/`CLAUDE.md:17`); for prose acceptance there is no runnable behavior to cover. This is exactly how every prior prose-only pipeline shipped with the writer profiles as they stand — so the writer profile is **not** edited by #86. (Residual: the writer profile's test-contract wording does not itself carve out this case; see Dropped + Open Questions + Risks. The path nonetheless works because the reviewer is the gate and the ban is in force, as the precedents demonstrate.)
  3. **Verification (reviewer side) — already supported, no change needed.** The `code-reviewer` verifies each prose task's Acceptance by **direct inspection of the shipped profile prose** (diff inspection, greps, byte comparison) with captured evidence in `## Behavior verification`, and runs the pre-existing guardrail suite unchanged — the mode `123`/`134`/`137` used and that `code-reviewer.md` step 3 (line 35, "anything a user or downstream consumer can see") + step 4 (line 45, "no guardrails convention → your step-2/3 judgment stands") already authorize. No reviewer-profile change is needed for the verification path. (The output-rules enforcement items added to `code-reviewer`/`docs-reviewer` per Topic 4 are themselves verified the same inspection way; the reviewer must not reject #86's own prose tasks for lacking a test — see Risks.)

- **Dropped from #86 (owner scope decision):** making the inspection affordance **explicit** in the writer profiles — i.e. adding an inspection-only branch to `code-writer-tdd.md`/`code-writer-e2e.md`'s test contract. That is a **general tool change** beyond #86's output-rules spec (it governs every future prose/instruction-only code task, not the output rules #86 is about), and it is **not required** for #86 to ship: prior prose-only pipelines (123/134/137/90) all shipped with these profiles unchanged. #86 edits the writer profiles **only** for Rule 1 / Rule 2 (its actual subject). The affordance is deferred to a separate effort (recorded in Open Questions).

- **Rationale (why decision-only is feasible, not merely less robust):** Decided on this repo's own history, not assumption. The repository has run this exact situation repeatedly: it tried the forbidden structural-test path (`122`, 06-13/15), then **deleted those tests and forbade them** (`433578b`, 06-16, the origin of `AGENTS.md:17`/`CLAUDE.md:17`), and every prose-only Code phase before and after that ban (`123`, `134`, `137`, `90`) shipped by reviewer inspection with **zero** test files **and with the writer profiles unchanged**. That last fact is what makes decision-only *feasible*, not just acceptable: the path does not depend on a new writer-profile affordance — it has shipped, repeatedly, without one. The reviewer profile already authorizes the inspection mode (`code-reviewer.md` step 3 + line 45), so verification is concrete and evidence-bearing, satisfying the spec's enforcement model (Req 12: a *seen* property is checked at the gate; detection is judgment, not a mechanical scan — consistent with Out-of-Scope #2/#4). The blocker named a real *literal-reading* tension in the writer's unconditional test-contract wording, but the empirical record shows the gate + the ban already resolve it in practice; closing the wording gap explicitly is a robustness improvement, not a precondition for #86 to ship.
  - **Why the wording gap is left to a separate effort (owner scope decision).** Amending `code-writer-tdd.md`/`code-writer-e2e.md` to add an inspection-only branch is a **general tool change**: it changes how the tool handles *any* prose/instruction-only code task, which is outside #86's output-rules spec. #86's subject is the two output rules; it edits the writer profiles only to add Rule 1 / Rule 2 (Topics 1, 4, 7), not to alter dispatch or verification. Keeping #86 to its subject means the writer-contract amendment is deferred (Open Questions), and #86's own prose tasks rely — as the precedents did — on the design-level decision + the ban + the reviewer's authorized inspection mode. This keeps the change a pure verification/dispatch **decision** with no tool/profile modification, and avoids smuggling a general-purpose tool change into a feature-scoped pipeline.

- **Self-consistency with Topics 1, 4, 7:** Topic 7's "ships no code and no runtime mechanism… verification by reading the profiles" is now backed by a concrete, executable path (the `123`/`134`/`137`/`90` precedent + the reviewer's already-authorized inspection mode), closing the Plan-phase blocker **without any new edit beyond what Topics 1/4/7 already specify**. #86's edited file set is unchanged from Topics 1/4/7: the producing/reviewing profiles, edited only for Rule 1 / Rule 2. Topic 8 adds **no** new files to change and **no** writer/planner edit — it is a dispatch/verification decision (type prose tasks `tdd`; verify by inspection at the gate, no test) that the `code-plan-writer` and `code-reviewer` realize using their profiles **as they stand**. It therefore does not alter Topics 1–7's decisions or scope.
...
- **(Topic 8 — DEFERRED to a separate effort, outside #86 per owner scope decision) Make the
  inspection affordance explicit in the writer/planner profiles.** A future, general-purpose change could
  add an inspection-only branch to `code-writer-tdd.md`/`code-writer-e2e.md`'s test contract (admitting a
  test-less, inspection-verified acceptance for prose/instruction tasks, citing the
  `AGENTS.md`/`CLAUDE.md` no-structural-tests rule), modeled on the existing "No guardrails convention —
  proceed" branch (`code-writer-tdd.md:42`) and the precedent review language (123/134/137); and
  optionally a one-line note in `code-plan-writer.md` that a prose/instruction-only task is typed `tdd`
  and verified by inspection. Constraints for that effort: scope the branch to *acceptance with no
  runnable behavior* (not a general license to skip tests) and keep the guardrail-gate obligation intact.
  **This is explicitly NOT part of #86** — it governs every prose/instruction-only code task, not the
  output rules #86 is about. #86 ships without it, exactly as prior prose-only pipelines did; the residual
  (the writer's test-contract wording still has no explicit test-less branch) is recorded under Risks.
...
- **(Topic 8) Residual: with the writer profiles unchanged, the test-contract wording still has no
  explicit test-less branch.** #86 ships decision-only (owner scope decision): it does NOT amend
  `code-writer-tdd.md`/`code-writer-e2e.md` to carve out inspection-verified prose acceptance. So the
  dispatched `code-writer-tdd` for #86's own prose tasks reads a test contract that, taken literally, has
  no test-less branch. **Why this is an accepted residual, not a defect:** every prior prose-only pipeline
  (`123`/`134`/`137`/`90`) shipped under the same unchanged wording, verifying by reviewer inspection with
  no test — so the path is empirically feasible. The resolution lives at the gate (the `code-reviewer`
  verifies by inspection — its already-authorized mode) and in the `AGENTS.md:17` ban (which forbids the
  only "test" that would otherwise satisfy a literal reading). The writer is expected to make the prose
  edits and not manufacture a forbidden structural test, exactly as the precedents did. Making the
  affordance explicit in the writer profile is the deferred general tool change (Open Questions) — a
  robustness improvement, not a precondition. Downstream phases must **not** treat the residual as license
  to add a structural prose-test (Out-of-Scope #2; `AGENTS.md:17`).
