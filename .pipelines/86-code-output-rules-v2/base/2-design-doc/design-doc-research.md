...
- **Options considered:**
  ...
  5. **Chosen — type prose tasks `tdd` (the only value the current template/router accepts for a non-e2e task, and 122's precedent on pure prose), verify by inspection at the gate (the universal mode, per 123/134/137/90), add no test, AND close the writer/planner profiles' unconditional-test-contract gap — which is in-scope precisely because those profiles ARE this feature's own deliverable surface.** See Decision.
...
- **Decision:** **This feature's Code phase types its prose profile-edit tasks `tdd`, ships the edits, and verifies them by `code-reviewer` inspection at the existing review gate adding no test (the verification mode every prose-only pipeline in this repo has used) — and the same change that ships the output rules ALSO closes the writer/planner test-contract gap that produced the blocker, because the writer/planner profiles are this feature's own deliverable surface.** Concretely:
  1. **Dispatch (planner side).** The `code-plan-writer` keeps the existing `Type: tdd | e2e` field, and a prose-only profile-edit task is typed **`tdd`** — the only non-e2e value the current template (`code-plan-writer.md:54`) and strict router (`4 - code.md:3,35`) accept, and the choice `122` made for its own pure-prose tasks. Its Acceptance is written as **observable, inspectable properties of the shipped profile prose** (e.g. "`code-writer-tdd.md` carries the canonical Rule 2 statement and the narrower `code-writer-tdd.md:33` line no longer exists"), NOT as runtime behavior. No new `Type` value and no third writer is introduced — the routing machinery (`4 - code.md` step 3.1) is unchanged.
