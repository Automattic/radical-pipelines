...
24. Guardrails is **not locally overridable**: it is a shared, must-hold-for-everyone contract, not a developer-local-runtime convention. It is kept non-overridable **by omission** — the shipped `load.md` `## Local overrides` section is a generic stub that does not enumerate an overridable subset, so guardrails is not added to any such subset and there is nothing to opt it into. This change adds **no new override mechanism and no new override behavior**: it does not author the "ignored, committed value used, run output warns" enforcement (that detailed machinery is not present in the shipped loader — it lives only in unshipped #91 pipeline artifacts, and authoring it is out of scope here). The only edit to `## Local overrides` is at most a short sentence noting guardrails is shared / committed-only, attached to the existing stub; it adds no enumerated subset and no override-enforcement text.
...
7. **Any new mechanism or enforcement behavior for local overrides.** Guardrails is non-overridable by omission (it is simply never added to an overridable subset). Authoring the detailed overridable-vs-shared guidance and the "ignored / committed-value-used / warn-on-override" enforcement into the shipped `load.md` is **explicitly not in scope** — that machinery exists today only in unshipped #91 pipeline artifacts, and porting it in would re-open #91 scope. Also out of scope: **README / website human-facing documentation** (owned by the pipeline's docs phase, not this spec).
...
11. **Local overrides — non-overridable by omission.** `load.md`'s `## Local overrides` section is not turned into an enumerated overridable subset, and guardrails is not added to one; at most a short sentence notes guardrails is shared / committed-only. No new override mechanism and no "ignored / committed-value-used / warn" enforcement text is authored (none exists in the shipped stub to extend). (Requirement 24.)
