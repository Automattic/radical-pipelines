...
## Addendum: README scope correction (found during phase 2 design)

Q4/Q5 concluded `README.md` was clean ("no singular doc-agent NAMES") and listed it as out of scope. That was wrong. `README.md:112` — the Pi-package install list enumerating every shipped agent profile — names all four documentation-concept agents in their OLD singular form: `doc-plan-writer`, `doc-plan-reviewer`, `doc-writer`, `doc-reviewer` (verified via `grep -noP '(design-)?doc-[a-z-]+' README.md`). The four `design-doc-*` profiles on the same line are the protected phase-2 concept and stay. Leaving line 112 singular would re-introduce the exact split this change eliminates in the project's most-read file, and AC#5 would not catch it because README was outside its scope.

Correction applied here: `README.md` removed from the out-of-scope listing; brought into Requirement 6 (it is a derived copy of the agent names, like `.rp.md` and `website/demo.js`) and the matching AC; and added to the AC#5 grep file list (`skills agents .rp.md website .changeset README.md`). README's already-plural mentions correctly stay: the "Docs" phase label at `README.md:32` and `docs-summary.md` at `README.md:157`. AC#5 re-verified with README included: 164 before → 161 after the three reworded Requirement-8 occurrences → 0 after the anchored rename; protected `design-doc`/`Design Doc` (245→246, the +1 being only the disambiguation) and `document`/`documentation` (124) untouched, no `design-docs` corruption, no `docss`.
