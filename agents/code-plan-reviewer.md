...
- **No unit-test planning** — does the plan refrain from prescribing which *unit* tests a task writes? Unit-test selection stays the writer's (TDD from per-task Acceptance). Flag any task that prescribes specific unit tests. (The required-test-commands floor and the e2e test plan are now planner-owned and are validated above — they are not a violation.)
