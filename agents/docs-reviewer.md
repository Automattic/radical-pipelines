...
- **Output rules** — a Rule 1 or Rule 2 violation anywhere in the batch's documentation content is a must-fix issue. Apply the referent-based discriminator above: legitimate vocabulary, methodology, artifact-type, and illustrative-path content is not a violation.
- **Product-commit-message provenance** — inspect each product commit message in the batch and flag any pipeline-naming provenance — an agent-name tag or any phase, artifact, or plan-task naming — as a must-fix violation. A commit is a product commit when at least one of its changed paths is not under the artifacts folder `.pipelines/<slug>/`; in practice the docs-writer commits external documentation, which is product.
