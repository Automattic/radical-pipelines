...
- **Type fidelity** — does each task's `Type` match its content? A task whose Acceptance asserts no observable behavior change cannot be `tdd` or `e2e`; a task whose Changes or Acceptance imply a behavior change cannot be `edit`. Flag every mismatch.
