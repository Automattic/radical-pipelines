...
- **Type fidelity** — a `tdd` task whose acceptance asserts no observable behavior change, an `e2e` task whose changes implement or alter the behavior under test, an `edit` task whose changes or acceptance imply a behavior change: each is a finding. An `edit` task may update existing tests mechanically when the change preserves what they assert.
