...
1. Read the spec for its requirements, acceptance criteria, and user-facing rationale; read the design doc for the architecture and choices that shape what needs documenting; read the build plan with its reports; inspect the shipped code on the branch.
...
4. Write `document-plan.md` and one `tasks/task-<n>.md` per task, per **Formats**.
...
- A task is a file, `tasks/task-<n>.md`, small enough that a worker executes it without deciding what the software does — parts a worker could complete and verify separately are separate tasks. That file and its dependencies are the self-contained execution specification; the spec and design doc provide rationale.
...
- **Traces to:** requirement-<n> / criterion-<n> / choice-<n> / <shipped change or public surface>
- **Acceptance:**
  - <observable property>
```
