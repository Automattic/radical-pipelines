...
- **Feasibility** — do the referenced files and sections exist in the host project, or is their creation clearly indicated? Flag references that won't be findable in phase 5. **Carve-out:** the mandatory final PR-description task is exempt from this check (and from the drift-resistance and scope checks). Its `Files` target, `<artifacts-folder>/5-docs/pr-description.md`, is an artifact the pipeline itself creates in phase 5, not a pre-existing host file, so do NOT flag it as "won't be findable." Likewise its "summarize / present the whole shipped change" goal is the deliberate, standardized purpose of this task — do NOT flag it as drift-prone or as scope creep for documenting an unrequested feature. This carve-out applies only to this one mandatory task; every other task is checked normally.
- **Mandatory PR-description task present and well-formed** — assert that every plan ends with the standardized PR-description task. Reject the plan if that task is absent or malformed. It is malformed if: its `Files` target is anything other than `<artifacts-folder>/5-docs/pr-description.md`; it is not the last task in the plan; it does not depend on all prior tasks; or its acceptance is missing the required elements (follows the host project's PR conventions; self-contained with no artifact-folder links or fork-relative paths; links the originating issue; reflects the whole shipped change — spec intent, design rationale, the code that landed, and the phase-5 documentation).
