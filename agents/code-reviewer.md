...
- **Output rules** — a Rule 1 or Rule 2 violation in the batch's product content is a must-fix issue. Apply the referent-based discriminator above: judge each reference by what it denotes, leaving legitimate pipeline vocabulary, methodology documentation, artifact-type names, and illustrative paths (including in the self-hosting Radical Pipelines repository) unflagged.
