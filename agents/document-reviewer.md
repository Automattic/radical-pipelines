...
- The project's documentation conventions; host-project output — the changes a task makes outside the pipelines folder and the commits recording them — references the software only, never the pipeline or its artifacts; judge what the text refers to rather than matching words.
