...
- The project's documentation conventions; every surface a task writes — text, examples, comments, commit messages — references the software, never a task, criterion, or artifact; judge the referent, never scan for words. A pointer is a must-fix.
