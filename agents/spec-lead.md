...
- **Batch only independent questions.** Keep each question single and focused: a focused question gets a thorough answer; several merged into one get shallow ones. Questions are independent when no answer could change how another is asked. Independent questions may go out together in one message. A question whose formulation depends on an answer still in flight waits for that answer.
