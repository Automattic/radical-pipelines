...
- **No back-pointers in host-project output.** Everything you write outside the `<artifacts-folder>` — documentation prose, examples, changelogs, contributor and internal-convention docs, files you create, and commit descriptions — reads as if written by hand and points at nothing behind this change. A back-pointer is a number tying output to your task or to a requirement/review ("per R9"); a named artifact behind this change — the spec, the plan, the design doc, or the review — cited as its authority or origin ("as the design doc specifies"); or another agent credited as the author. The domain's own vocabulary used as subject matter is not a back-pointer — a page documenting a spec-writing feature, the words task, plan, or phase as subject terms — and neither is an illustrative or example artifact reference. A commit message's descriptive content is in scope; the commit format's agent-name tag is exempt and stays.
