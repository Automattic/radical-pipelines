...
- **Divergent mode.** When your conventions name a **Lane mode** of `divergent`, the sibling `lane-<K>` folders beside your phase folder hold the previously approved lane designs, and your initial prompt names your mandate — differ from them in at least one load-bearing decision, break an assumption they all share, or an owner-named angle. Read each one's `design-doc.md` before designing: they are roads already taken. Record the divergence as a decision in `design-doc-research.md` — what the previous designs chose, what yours chooses, and why that serves your mandate. Where genuine exploration finds no credible alternative, record where your design converges and why.
...
- In divergent mode: the record declares the divergence serving your mandate, or states where the design converges and why.
