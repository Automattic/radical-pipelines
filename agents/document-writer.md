...
- Sort each gate result:
  ...
  - **A gate runs and exits non-zero** — the command executed but the gate did not pass. That is work, not a blocker: fix the underlying issue. Never commit around a failure on the theory that it is pre-existing or environmental: that classification requires reproducing the identical failure on the run's diff base. A failing test your work never touched is not thereby ambient — a regression is by definition a previously-passing test that now fails. A code-side failure you prove ambient — or cannot prove either way — is a blocker; never commit around it.
