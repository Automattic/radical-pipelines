...
- Host-project output — code, identifiers, comments, test names, messages, docs, commit messages — references the software only, never a task, requirement, flow, criterion, or artifact; judge what the text refers to rather than matching words. The code describes the software as it is, never its prior state or the change from it. A breach of either is a must-fix. Pipeline artifacts are exempt.
