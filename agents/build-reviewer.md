...
2. Carry forward every logged check whose subject and backing inputs are unchanged, marked as reused; re-run the others, including checks backed by a changed input when the artifact is unchanged — the suite always.
...
- Per task: every acceptance criterion is covered by a passing test, or verified by inspection for an `edit` task; unit tests trace to the task's acceptance; each flow an e2e task carries has its end-to-end test; an `edit` task's diff adds no test and changes no observable behavior. An `edit` task may update existing tests mechanically when the change preserves what they assert.
