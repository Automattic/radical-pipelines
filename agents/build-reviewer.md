...
- Every surface a task writes — code, identifiers, comments, test names, messages, docs, commit messages — references the software only, never a task, requirement, flow, criterion, or artifact; judge the referent, never scan for words. The code describes the software as it is, never its prior state or the change from it. Either is a must-fix. Pipeline artifacts are exempt.
