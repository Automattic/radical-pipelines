...
- **PR-description artifact.** When the assigned task is the PR-description task — producing `pr-description.md`, whose entire content is used verbatim as a pull-request body — three additional constraints apply. They govern only this task; your single-task, accuracy, convention, and "do NOT touch source code" behavior is otherwise unchanged.
  - **(i) Follow the host project's PR conventions, discovered at produce-time.** Discover them exactly as you discover the host's documentation and verification conventions: use a PR template if the host provides one; otherwise the host's observed / de-facto PR conventions, discovered by inspecting recent merged pull/merge-requests in the host's tracker; otherwise a sensible generic PR body. Do NOT mandate fixed section names. Express the generic fallback as content categories rather than required headings: a short summary of what shipped and why, a breakdown of the concrete changes, how it was verified, and the issue reference.
  - **(ii) Keep the artifact self-contained, with R3 over R5.** Because the whole content is reused verbatim as a PR body, it must stand alone: include no references into the pipeline's artifact folder and no fork-relative paths (in `artifacts-in-fork` mode the upstream PR viewer never sees the fork's `.rp/` tree, so such links break). It MAY link to publicly resolvable targets, such as the originating issue or an absolute "Generated with Claude Code"-style URL. Where the host's observed PR conventions include a reference to the artifact folder or any fork-relative path — for example a "How this was produced" line citing a `.rp/pipelines/...` path — **R3 takes precedence over R5: keep the provenance mention, strip the fork-relative path.**
  - **(iii) Reference the originating issue tracker-agnostically.** Reference it in the form the host's PR conventions and tracker use: where the tracker supports an auto-close keyword and the host uses it, use it (for example a GitHub `Closes #N`); otherwise a plain link or identifier that resolves to the issue in the host's tracker. Do NOT hard-code a GitHub-specific keyword. Source the issue identifier from `<artifacts-folder>/0-prompt/prompt.md` (its `Source issue: ...#N` line), which every pipeline inherits.
