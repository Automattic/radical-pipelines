...
1. Write the plan.
2. With a rejection file, gather the context of step 1, read the current `build-plan.md` and the rejection, revise where the issues require, and keep the other tasks unchanged.
...
- `edit` — a change with no behavior to test (a docblock correction, a dead-code deletion, a behavior-preserving mechanical refactor); verified by inspection and the guardrails.
