...
- **Inherited phase**: the highest-numbered phase to inherit, by folder name (`0-prompt`, `1-spec`, `2-design-doc`, `3-plan`, `4-code`, `5-docs`). The new pipeline continues from the next phase or revises the inherited phase. Pick `0-prompt` to start the new pipeline over from scratch — only the prompt is inherited.
