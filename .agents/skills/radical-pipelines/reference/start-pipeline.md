...
Before executing these steps, make sure you have loaded and verified the project conventions (see `loading-conventions.md`). If the active CLI is Pi, also verify the required phase agent definitions by checking repository-local `.pi/agents/` first and user-local/global `~/.pi/agent/agents/` second. If any required convention or Pi agent definition is missing, stop and run the setup flow in `setup.md` before continuing. Each step below applies one of those conventions.
