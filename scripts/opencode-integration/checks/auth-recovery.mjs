...
export async function run(ctx) {
  ...
  await runCheck(results, "retry 2 — a re-spawn on the authenticated model runs successfully", async () => {
    ...
    const result = await driveToolCall(server, orchestrator.id, "rp_spawn", { name: "auth-recovery-respawn", agent: "helper", model: "stub/stub-model", directory: projectDir, prompt: "say hello", run: "auth-recovery-run" });
  ...
  await runCheck(
    ...
    async () => {
      const spawn = await driveToolCall(server, orchestrator.id, "rp_spawn", { name: "auth-cause-child", agent: "helper", model: "stubnoauth/stub-model", directory: projectDir, prompt: "say hello", run: "auth-recovery-run" });
      ...
      const status = await driveToolCall(server, orchestrator.id, "rp_status");
