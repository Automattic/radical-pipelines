...
- **Phase 2 (Design doc)** produces `design-doc.md` from the spec. In the autonomous workflow, a fresh `design-writer` reads `spec.md`, investigates the codebase, and writes `design-doc.md`; then a fresh adversarial `design-reviewer` either approves or sends specific issues back for revision. The loop continues until approval. The `radical-pipelines-design` team template wires these two agents together for project-convention-driven use.
