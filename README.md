...
The skill is generic — each project defines its own conventions for things like repository ownership, where Radical Pipelines metadata may be written, the task source, existing work checks, pipeline slug format, worktree commands, branch naming, artifact folder location, commit format, fork ownership policy, and how teams of agents are spawned. These conventions can live in any of these places (checked in order):
...
If required conventions are missing when a workflow starts, Radical Pipelines stops before running the pipeline and offers an interactive setup flow. Setup asks for the missing convention details once, explains which answers are shared project guidance and which are CLI-specific Radical Pipelines guidance, and can write reusable Markdown for the active CLI after confirmation. Future runs read the stored conventions instead of asking ownership questions again. In owned repositories, Pi setup writes only Pi conventions to `.pi/rp.md` and Claude Code setup writes only Claude Code conventions to `.claude/rp.md`; in not-owned repositories, setup should use an owner-approved external or local-only conventions location instead of adding project noise.

Repository ownership is a required setup convention. In an **owned repository**, a project may choose to track Radical Pipelines support files and artifacts such as `.pipelines/`, `.pi/rp.md`, or `.claude/rp.md`, and may update `.gitignore` for local worktree folders. In a **not-owned repository**, setup records the owner's fork remote/URL and branch/worktree policy, and all Radical Pipelines work and commits happen in that fork. Radical Pipelines convention files should not be added to the upstream project repository, `.gitignore` must not be modified there for Radical Pipelines, local-only ignores should use `.git/info/exclude`, and versioned/multi-user artifacts must live in a separate repository.

Shared project conventions include repository ownership, task tracking, pipeline slug format, pipeline artifact folder location, commit format, and fork/open-PR ownership policy. Claude Code conventions include Claude Code worktree commands, branch creation, team spawning, and Claude Code prerequisites. Pi conventions include Pi packages and plugins, Pi worktree commands, branch creation, pi-teams spawning, provider/model recovery, and Pi agent setup. Do not mix Claude Code conventions into `.pi/rp.md`, and do not mix Pi conventions into `.claude/rp.md`.
...
The orchestrator loads and verifies conventions before launching phase agents. It applies the stored ownership convention before creating files: owned repositories follow their configured persistence policy, while not-owned repositories must already be operating in the configured fork worktree. If ownership, fork policy, or artifact persistence is unclear, it runs setup instead of creating `.pipelines`, `.pi`, `.claude`, `.gitignore`, `AGENTS.md`, or other personal automation files in the target repository. When it spawns a phase agent or team, it passes the resolved pipeline slug, artifact folder path, exact artifact paths for that role, and the role-specific host-project conventions listed in the agent profile. Phase agents can commit according to the commit convention; fork verification and later open-PR behavior are orchestrator responsibilities.
