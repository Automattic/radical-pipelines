...
There is no `npm publish` — the root package is `"private": true` and consumed direct-from-git, and although `packages/opencode/package.json` is an npm-publishable manifest the release flow does not publish it — but a release now produces a `v<version>` git tag and a GitHub Release via CI.
