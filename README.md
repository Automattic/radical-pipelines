...
A developer can override a restricted subset of conventions for their own working copy by placing a git-ignored `.rp.local.md` alongside the committed `.rp.md`. The local file wins per named unit, the committed file is inherited wherever the local file is silent, and keyed lists (such as the `Agent models` bullets) are map-merged so a local entry replaces only its matching key while the rest are inherited. Only agent models, the health-monitoring cadence, and the Issues access-mechanism are overridable; shared and tool-forced conventions (such as the commit format, artifact folder, pipeline slug, branch and worktree naming, the Issues tracker identity, and tool-dictated command forms) are not. Because `.rp.local.md` is git-ignored, it is never committed and never affects other contributors. See the [local overrides reference](./skills/radical-pipelines/reference/conventions/local-overrides.md) for the full mechanism.
