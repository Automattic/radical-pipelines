...
- **Output that reads as your own.** Every run holds the product it ships — code, tests, documentation, and commit messages — to two always-on rules, with no opt-out and nothing for you to configure per run or per project. A change leaves comments and prose it did not touch exactly as they were, and the shipped product reads as if written by hand, with no trace of the pipeline that produced it. Both rules are enforced at the same per-phase review gate every other quality check passes through, where a violation blocks the phase until it is resolved.
