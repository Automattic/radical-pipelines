...
The orchestrator creates one `pi-teams` team per pipeline and spawns the phase agents at runtime, following the project configuration.
...
The skill is generic — each project defines its own configuration for things like the task source, existing work checks, pipeline slug format, worktree commands, branch naming, artifact folder location, and how teams of agents are spawned. A project's shared configuration lives in a committed `.rp.md` file, populated by the interactive setup flow; an individual developer can optionally layer a restricted subset of local overrides on top of it (see below).
...
A project's committed [`.rp.md`](./.rp.md) contains the shared configuration (issue tracking, pipeline slug format, artifact folder, commit format, Linear updates, push behavior) together with any tool-specific configuration the active CLI requires (worktrees, branch names, team spawning, agent models, health monitoring). A normal single-CLI consumer needs the shared rules plus the reference file for its active tool. This repository is the unusual case: as the only multi-CLI consumer of Radical Pipelines, it dogfoods both CLIs at once, so the reference files cover both Claude Code and Pi.
