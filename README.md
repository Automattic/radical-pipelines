...
- A session that activated the skill gets it back after opencode's context compaction. Compaction summarizes the transcript and drops the instructions the assistant was given rather than told — the activated skill and the files read from its folder among them. Every model request shows the plugin the session's active context; it records the skill activation and every `read` of a skill file or project convention file (`.rp.md`, `.rp.<tool>.md`, `.rp.local.md`) it sees there. A request that no longer carries a recorded activation is continuing from a checkpoint: the plugin tells the session, before anything else, to load the skill again with the `skill` tool, read again the recorded files the request lacks, and recompute state with `rp_status` and `rp check`. Loading through the skill makes the session follow the skill's own load path again, including the project files its conventions name; once the activation is back in its context, the instruction stops. The checkpoint summary request itself is instead asked for a `## Pipeline run` section holding what the owner settled at triage and what is in flight. A session the daemon holds no record for — it restarted since the checkpoint — is read once from its stored history. `rp_status` lists the sessions that activated the skill under `skillReloads`, with the files they are told to read again; a history that cannot be read records `skill.reload.unreadable` and is read again on the next request.
