...
- **Phase 5 (Docs)** ships `doc-writer` and `doc-reviewer` agent profiles. The orchestrator dispatches one fresh `doc-writer` per task from `doc-plan.md`, sequentially, because all writers share the pipeline branch's single working tree. Each writer reads three sources of truth — the assigned task block (what to document, for whom), the spec and design doc (why this exists and why it is shaped this way), and the shipped code from phase 4 (what actually exists, names, signatures, paths, behavior) — and synthesizes them into documentation that matches its stated audience. Every concrete claim is verified against the shipped code; any documentation gates the host project's verification convention enumerates must pass before the writer commits (many projects enumerate none and rely on the reviewer's accuracy spot-check). The phase also produces the pull-request description (`<artifacts-folder>/5-docs/pr-description.md`) — a single self-contained Markdown file usable verbatim as a PR body. It is not a separate flow: the `doc-plan-writer` appends one mandatory, standardized PR-description task as the always-last entry of every `doc-plan.md`, depending on all prior tasks, so it rides the same dispatch, sequential-commit, and review machinery as the rest of the docs. The `doc-writer` assigned to it follows the host project's PR conventions discovered at produce-time, keeps the artifact self-contained (no links into the artifact folder, no fork-relative paths), and links the originating issue tracker-agnostically. After every doc-writer in the batch commits, a single fresh `doc-reviewer` reviews the whole batch against `doc-plan.md`, the spec, the design doc, and the shipped code — and, when the batch includes the PR-description task, additionally checks that the artifact reflects the whole shipped change, links the originating issue, and stays self-contained. Those checks ride the phase's single approve/reject gate: an artifact problem is reported as an issue tagged to the PR-description task, with no second approval or terminator file. On rejection, issues are tagged per task; the orchestrator re-dispatches only the affected tasks — always including the PR-description task, run last, so the description is re-produced against the latest docs — and the reviewer runs again, until approved. Phase 5 is not complete until `pr-description.md` is committed alongside the `docs-review-approved.md` terminator.
