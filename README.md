...
The skill is generic — each project defines its own conventions for things like the task source, existing work checks, pipeline slug format, worktree commands, branch naming, artifact folder location, and how teams of agents are spawned. A project's conventions live in a committed `.rp.md` file, populated by the interactive setup flow; an individual developer can optionally layer a restricted subset of local overrides on top of them (see below). The same `.rp.md` also declares the project's guardrails — its mandatory verification gates (see below).
...
A project's committed [`.rp.md`](./.rp.md) is organized as two top-level sibling sections: `## Conventions` (the project's pipeline conventions — issue tracking, pipeline slug format, artifact folder, commit format, worktrees, branch names, team spawning, health monitoring, and so on) and `## Guardrails` (the project's verification gates). A guardrail is an exact command (such as `npm test`), judged pass/fail solely by its exit code and mandatory within the phase(s) it applies to; the only phase targets are **code** and **docs**, and a guardrail may apply to one or both. Guardrails are tool-agnostic — the same command runs regardless of the active CLI, so there are no per-tool guardrail variants — and they are optional: a project may declare none, which is a complete, valid state. The [convention loader](./skills/radical-pipelines/reference/conventions/load.md#guardrails) carries the authoritative definition. This repository dogfoods its own guardrails: `npm test` (code phase) and `node scripts/validate-changesets.mjs` (code and docs phases).

The per-tool split applies to *conventions* only. Tool-agnostic conventions (issue tracking, pipeline slug format, artifact folder, commit format, Linear updates, push behavior) live in `.rp.md` itself, while conventions that depend on the active tool (worktrees, branch names, team spawning, agent models, health monitoring) live in their own per-tool reference files, loaded conditionally for the active CLI. A normal single-CLI consumer therefore reads `.rp.md` plus the one tool block its CLI uses. This repository is the unusual case: as the only multi-CLI consumer of Radical Pipelines, it dogfoods both CLIs at once, so its per-tool conventions cover both the Claude Code and the Pi tooling side-by-side.
