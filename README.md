...
The orchestrator loads and verifies conventions before launching phase agents. When it spawns a phase agent or team, it passes the resolved pipeline slug, artifact folder path, exact artifact paths for that role, the absolute worktree root that anchors every path the agent writes (passed to every spawned agent), the pipeline branch the agent commits to (passed to the agents that commit), and the role-specific host-project conventions listed in the agent profile. Phase agents report a blocker when required context is missing instead of inferring paths from generic examples.
