...
- A session that activated the skill keeps it across opencode's context compaction. Compaction summarizes the transcript and drops the instructions the assistant was given rather than told — the activated skill and the files read from its folder among them — so the plugin treats them the way opencode treats instruction files. Every model request shows the plugin the session's active context; it records the skill activation and every `read` of a skill file or project convention file (`.rp.md`, `.rp.<tool>.md`, `.rp.local.md`) it sees there. A request that no longer carries a recorded activation is continuing from a checkpoint: the plugin re-supplies the skill and the recorded files the request lacks as system text, current as on disk, together with the agents the session spawned and the health loops targeting it, and tells the session to recompute state with `rp check` and `rp_status` before acting. The checkpoint summary request itself is instead asked for a `## Pipeline run` section holding what the owner settled at triage and what is in flight. A session the daemon holds no record for — it restarted since the checkpoint — is read once from its stored history. `rp_status` lists the sessions that activated the skill under `skillReloads`, with the files re-supplied and any that could not be read; a history that cannot be read records `skill.reload.unreadable` and is read again on the next request.
