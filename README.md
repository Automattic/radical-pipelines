...
The pipeline is **autonomous by default, assisted when needed.** After triage, questions are reserved for owner escalations and approval of incoming work. While waiting for agents, the orchestrator ends its turn and resumes on incoming messages or scheduled health ticks. The Spec and Design doc phases can instead run in assisted mode.

It is **inspectable.** Every phase produces artifacts your team can review before the final PR. Authored changes retain their patches and both sides of changed files; [`rp diff`](./skills/radical-pipelines/scripts/README.md#diff) renders them even when source commits disappear. A checkpoint preserves that history after merging and on continuation.
...
- Use `rp_status` with `pipeline_slug` for health checks. Each `ledger` row contains only `sessionID`, `running`, `secondsSinceActivity` (whole seconds since the latest activity, sampled when the response is built), `pending` (undelivered inputs), and actionable `permissions`. Unknown health fields are `null`. `recentErrors` contains failure summaries: type, session when known, timestamp, and cause capped at 200 characters; successful lifecycle events are omitted. A stopped or inactive agent warrants investigation, not automatic recovery.
- Use `rp_status` with `session` to investigate an agent. It returns `pluginVersion`, identity and configuration (`name`, `agent`, `model`, `directory`, `pipelineSlug`), the raw `activity` timestamp, `currentTool`, `lastTurn` (`succeeded`, `failed`, or `interrupted`), `lastSend`, and `lastText` (the newest assistant text, capped at 200 characters; the search depth when none was found; or `null` when the transcript holds none). Its `recentErrors` retains full diagnostic events, including their `info` or `error` level. An unscoped call returns the plugin version and detailed rows for every RP session, without reading transcripts.
- Activity is the latest of opencode's `updated` (which moves when a session receives input), observed tool or model progress, and raw provider bytes. `lastTurn` and `lastSend` live in daemon memory: after a restart they are absent until observed again, and activity falls back to `updated` until progress is observed. Per-session server reads cover only the selected scope. Scoped diagnostic events concern its recorded or live sessions plus events naming no session; unscoped calls include every event. Every status response includes `readFailures`: unavailable or malformed reads leave the affected facts unknown rather than reporting a healthy or empty pipeline.
...
The skill is generic: each project records its conventions in a committed `.rp.md`. Its frontmatter, a JSON object, carries `"conventions": 4`, the version of the conventions format, so the loader can migrate older files or ask the owner to update the skill when a file is newer. If the file is absent or required conventions are missing, the interactive setup writes it only after the owner confirms the proposed content.
...
| Resources              | What the project's agents have available and how to use it                                 | No       |
