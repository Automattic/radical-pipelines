...
- A session that activated the skill gets it back after opencode's context compaction. Compaction summarizes the transcript and drops the instructions the assistant was given rather than told — the activated skill and the files read from its folder among them. Every model request shows the plugin the session's active context; it records the skill activation — a successful `skill` tool call, or the skill attached to a prompt — and every successful `read` of a skill file or project convention file (`.rp.md`, `.rp.<tool>.md`, `.rp.local.md`) it sees there. A request from a session that activated the skill, whose record holds anything the request no longer carries, is continuing from a checkpoint: the plugin tells the session, before anything else, to load the skill again with the `skill` tool and read again the recorded files it lacks, then recompute state with `rp_status` and `rp check`. Loading through the skill makes the session follow the skill's own load path again, including the project files its conventions name. As each item returns to the session's context it leaves the instruction, and a recorded file no longer on disk is not asked for; once nothing is lacking, the instruction stops. The checkpoint summary request itself is instead asked for a `## Pipeline run` section holding what the owner settled at triage and what is in flight. A session the daemon holds no record for — it restarted since the checkpoint, or the record was evicted — is read once from its stored history; a deleted session's record is dropped. `rp_status` lists the sessions that activated the skill under `skillReloads`, with the files they are told to read again; a history that cannot be read, or reads back malformed, records `skill.reload.unreadable` and is read again on the next request.
