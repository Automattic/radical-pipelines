...
- Health-loop ticks skip recently active sessions, steer into sessions with no activity for two intervals, and remain observable through `rp_status`. A tick never duplicates a prompt still awaiting delivery — a parked queue copy facing a running session is promoted to steer delivery in place; when an injection is answered only by a failing turn (network outage, provider quota exhaustion), the loop backs off exponentially — up to ~8 intervals between probes — while still inspecting the target every tick and never delaying the steer that arms hung-session recovery, and resumes normal cadence on the first success; and a target stuck on a provably dead provider stream (an unconsumed steer against a tool call frozen mid-streaming) is interrupted so execution resumes with the pending monitor steer, while a target inside a long-running tool call is left alone.
