...
- `--json` emits machine-readable state. `challenges` has one entry per (challenge, target), with its state and `inScope`; entries for one target artifact share adjudication and resolution. `challengeResolved` is true when every target artifact is resolved. Pending challenges, including claims, reach the frontier in phase order only when the target's consumed inputs match its required package with current valid input approvals. Otherwise the phase walk reaches the target; its artifact line and `pendingChallenges` list the pending paths to carry into re-synthesis.
