...
- `Target:` is a comma-separated list, mirrored to `target` as a list even for one entry. `target-identity` requires one valid landing identity per target, in the same order; retained targets keep theirs. Both commands validate these fields; `check` reports invalid representations as `INVALID FRONTMATTER` before computing facts. Stamp validates new targets together under the [trigger definition](../reference/run/state.md#terms). A report's target must equal its own task, derived from its path; a failed report without `Target:` supplies it implicitly.
