...
- A pending claim on owner territory selects owner escalation; a constraint citing the claim as `Origin:` answers it. Constraints and proposals are challenges on their targets, pending until pinned and resolved by a current approving wave. Production follows phase order: `converge <artifact>` covers a missing or stale artifact, its pending challenges, and rejected closed waves. Consolidation remains `consolidate <artifact>`.
