...
- Identity is the first 12 hexadecimal characters of the body's git blob hash, byte for byte, for every pinnable file. Stamping preserves it. Symlinked paths are refused.
...
- `--lanes` declares, per artifact, the named review lanes and, after `|`, production lanes with their `after` dependencies: `"spec=security@<fingerprint>[materials=1-spec/spec.md+0-intent/intent.md]|event-driven@<fingerprint>,contrarian@<fingerprint><event-driven;build=fresh@<fingerprint>"`. Every named lane is `<id>@<fingerprint>`, matching the stamped `lane`; optional `materials=` lists the pipeline-relative files a filtered review lane receives and reviews. Without it, the lane reviews the full package. Components, paths, and delimiters are exact; artifacts, lane ids, and material paths appear once. A lane folder or review the declaration lacks is reported; `tasks` is reserved.
