...
- `--json` emits machine-readable state. `challenges` has one entry per (challenge, target), with its state and `inScope`; entries for one target artifact share adjudication and resolution. `challengeResolved` is true when every target artifact is resolved. Pending pairs reach the frontier in phase order.
