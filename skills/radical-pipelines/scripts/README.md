...
- `Target:` is a comma-separated list, mirrored to `target` as a list even for one entry. `target-identity` requires one valid landing identity per target, in the same order; retained targets keep theirs. Both commands validate these fields; `check` reports invalid existing frontmatter as `INVALID FRONTMATTER` before computing facts, while absent frontmatter leaves declarations pending stamp. Stamp validates new targets together under the [trigger definition](../reference/run/state.md#terms). Only failed reports have targets: their own task, derived from the report path and supplied implicitly when `Target:` is absent.
