...
- `Target:` is a comma-separated list, mirrored to `target` as a list even for one entry. `target-identity` stores the corresponding landing identities in the same order; retained targets keep theirs. Stamp validates new targets together under the [trigger definition](../reference/run/state.md#terms). A failed report without `Target:` uses its plan's task clause.
...
- `--json` emits machine-readable state. `triggers` has one entry per (trigger, target), with its state and `inScope`; `triggerResolved` is true when every target of that trigger is resolved. Pending pairs reach the frontier in phase order.
