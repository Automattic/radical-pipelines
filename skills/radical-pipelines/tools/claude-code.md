...
Name each instance `<profile> <slug>-<n>` (`<slug>-<lane>-<n>` in a lane), `<n>` counting that profile's instances in the pipeline. The spawn result's `agentId` is the address for messages; a helper's prompt carries its requester's address under **Requester**.
