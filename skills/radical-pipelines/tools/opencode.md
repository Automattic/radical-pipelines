...
- On health ticks, call `rp_status` with `pipeline_slug`. If the overview suggests a [health issue](../reference/conventions/health-monitoring.md#each-tick), inspect the affected agent with `session` before recovery.
