...
3. Render, per pipeline: pipeline slug, branch, live or merged, `origin` (issue; starts-from or re-attempts), configuration (workflow, target phase, lanes), per-phase state (missing · stale · in review · approved · complete), unresolved challenges, pending claims and owner escalations, waves this episode, the open correction branch.
