...
- **Wave** — one review of an artifact by every one of its lanes; numbered per artifact, shared by its lanes. A wave is valid on a package when its consumed members equal the artifact's pins in the tree and every declared lane records `approved` with `reviewed` equal to the package it received in members and identities; a filtered lane receives its selected projection. The reference package comes from the artifact for live state and from the root's recorded package for a closed lane. A valid wave is current when its package is required now.
