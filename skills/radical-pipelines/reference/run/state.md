...
- **Authored change** — what a commit introduces outside the pipelines folder: its diff against its parent (the empty tree for a root); for a merge, its diff against Git's default automatic merge of the same ordered parents. Identity is `git patch-id --verbatim` of the binary, rename-disabled Myers diff with three context lines and conflict-marker parent labels normalized, independent of local diff settings. Preserving that patch preserves identity across a rebase or cherry-pick. Each occurrence of a patch is a package member.
...
Current authored changes are the checkpoint in the pipeline's **base tree**, followed by the authored changes from base to inspected tip. The checkpoint, `changes/index.json`, is a JSON array of patch ids in commit order; absent means empty. Occurrences are numbered per patch from 1 across that sequence. A phase review's initial stamp records the current sequence as the prospective checkpoint and pins it.

`changes/<patch-id>.json` retains the first immutable material for a patch. A report or phase-review stamp records the material it names. Referenced material must be readable and valid. Its fields are:

- `patchId`; `patch`: the canonical diff bytes, base64-encoded.
- `files`: changed paths, each with `path`, `before`, and `after`. A side is null or `{mode, oid, bytes}`; blob `bytes` are base64, a gitlink has only mode and object id. Paths are repository-relative.
- `source`: observed `commit`, ordered `parents`, and diff `base` (null for a root). These addresses are provenance.
...
| `changes`   | task reports                 | the patch ids observed for `commits`, in order, omitting empty changes; retained while the declaration matches |
