...
A file pins exactly what it consumed, never its sibling record. A later phase never pins an earlier phase's record. A lane package consumed by another lane or its consolidated root includes the artifact, record, and approving reviews.
...
**What a review names** (`reviewed`): its review package. The implicit lane and a named lane without a `materials` filter name its artifact, record, and everything the artifact pins — its inputs, their approving reviews, the triggers it adjudicated, its lane inputs; a plan review adds every task; a build or document review adds the plan's package, every task, and every report. Task reports supplied to a plan review are context; only reports the plan pins enter `reviewed`. A filtered review lane names exactly its resolved material paths. A review whose pins are stale, that names less or more, or whose `lane` differs from the declared fingerprint, is stale. An artifact consumes an input with its **current approval**: every lane's review of the wave that approved it. A newer approval makes the artifact stale; incomplete pins means its stamp omitted a required input.
...
4. **Per phase, in order** — declared production lanes first, each a sub-pipeline of the root artifact (missing / stale / wave / adjudication; a lane waits until every `after` lane is approved and fresh with its complete required-input package, recursively; every lane approved and fresh → consolidate; a lane closes permanently when the root pins its artifact and record at the identities approved by one complete wave, plus those reviews); then the root artifact: missing → produce; stale → produce with the delta; unstamped or incomplete pins → stamp what its producer consumed; an unstamped review → stamp it; an invalid one → its reviewer finishes it; not approved → a review wave, or an adjudication when the wave closed with a rejection. Approved means every lane approved the current wave with fresh pins. In build and document, with the plan approved and fresh: an unstamped report of any attempt → stamp it; an invalid one → its worker finishes it; the next task — the lowest-numbered task whose dependencies are done and which is neither done nor held by an unadjudicated failure — as `blocked <phase>/T<n>` when its latest report is fresh and `blocked`; all done → the phase review wave; approved → the phase is complete.
