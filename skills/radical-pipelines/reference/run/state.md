...
- **Wave** — one review of an artifact by every one of its lanes; numbered per artifact, shared by its lanes. Its unfiltered `reviewed` package is its historical certificate. A wave is valid when every lane records a verdict over a concordant projection of that complete package. It approves that package when every verdict is `approved`, and is current when the package is required now.
