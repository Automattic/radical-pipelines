...
**What a review names** (`reviewed`): its declared materials. The implicit lane and a named lane without a `materials` filter receive the full package: its artifact, record, and everything the artifact pins — its inputs, their approving reviews, the triggers it adjudicated, its lane inputs; a plan review, every task too; a build or document review, the plan's package, every task, and every report. A filtered review lane names exactly its resolved material paths. A review whose pins are stale, that names less or more, or whose `lane` differs from the declared fingerprint, is stale. An artifact consumes an input with its **current approval**: every lane's review of the wave that approved it. A newer approval makes the artifact stale; incomplete pins means its stamp omitted a required input.
