...
2. **A pending challenge** targeting a phase within the target, whose target's inputs are each fresh with a current valid wave → work on its target. Otherwise the phase walk reaches the target first, and its re-synthesis carries the challenge.
