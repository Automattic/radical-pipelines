...
- **Challenge** — a file asking for work on its targets: a correction, an `unsatisfiable` verdict (a **claim**), or a fresh failed task report. A challenge targets what its evidence contradicts. A claim targets one clause: an id that exists when it lands, or an existing intent Goal, Constraint, or Decision. A failed report targets its task (`build-plan.md#T3`). A correction targets one or more of the four artifacts at the level the request states: a clause it names, else the whole artifact. After landing, resolution uses the recorded targets regardless of their current textual presence. State is per target artifact: several clauses of one artifact are adjudicated together, each dispositioned in the record. A challenge is **pending** on a target until that target **adjudicates** it by pinning it — the disposition, adoption or refutation, lives in the record — and **resolved** on it when the target has a current valid wave that names the challenge, or when a closed wave of the target corroborates an `unsatisfiable` verdict citing it as `origin` (escalated one layer up); the challenge is resolved when every target is. A claim is its lane's verdict: it persists until resolved, superseded (its target changed, or its lane reviewed again), or moot (the artifact it judged changed). A failed task report holds its task until adjudicated or until the task changes; a blocked one leaves it pending for you.
...
| `target`    | challenges                   | `<path>#<id>`; `<path>` for a correction; a list when several. Only challenges carry `Target:`, `target`, or `target-identity`. |
| `target-identity` | challenges                   | each target's identity when the challenge landed; a list when several    |
...
| `head`      | files with pins              | the commit a stamp with pins observed: the diff base for the next re-synthesis or artifact delta review |
...
2. **A pending challenge** targeting a phase within the target, whose target's required package is current — every input at its current identity, with the approving wave the table requires → work on its target. Otherwise the phase walk reaches the target first, and its re-synthesis carries the challenge.
...
5. **Complete** — every challenge and claim targeting a phase within the target resolved, every phase through the target has a current valid wave over a fresh artifact → close-out. A base that does not resolve is an error, never completion.
