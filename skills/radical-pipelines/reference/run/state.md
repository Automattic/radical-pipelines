...
- **Challenge** — a file asking for work on its targets: a constraint, a proposal, an `unsatisfiable` verdict (a **claim**), or a failed task report. A constraint or proposal targets one or more phase artifacts, whole or by clause. A claim targets one existing artifact clause or constraint file. A failed report targets its task (`build-plan.md#build-task-3`). After landing, resolution uses the recorded targets regardless of their current textual presence. State is per target artifact: several clauses of one artifact are adjudicated together, each dispositioned in the record. A challenge is **pending** on a target until that target **adjudicates** it by pinning it — the disposition lives in the record — and **resolved** on it when the target has a current valid wave that names the challenge, or when a closed wave of the target corroborates an `unsatisfiable` verdict citing it as `origin` (escalated one layer up); the challenge is resolved when every target is. A claim on owner territory is resolved by its owner's answer. A claim is its lane's verdict: it persists until resolved, superseded (its target changed, or its lane reviewed again), or moot (the artifact it judged changed); one its target adjudicated stays unresolved there until resolved. A failed task report stops being a challenge when its task changes before adjudication; a blocked one leaves its task pending for you.
- **Under experiment** — an artifact while a challenge unresolved on it is a failed task report or has one in its `origin` chain. `rp check` ends its `converge` and `review wave` lines with ` (experiment)`; their agents run under `Execution: experiment`.
- **Done-set** — the tasks whose latest report's `outcome` is `completed` and whose `reviewed` pins are fresh.
- **Authored change** — what a commit introduces outside the pipelines folder: its diff against its parent (the empty tree for a root); for a merge, its diff against Git's automatic merge of the same ordered parents, using the repository's effective merge configuration and attributes. Its patch id is `git patch-id --verbatim` of the binary, rename-disabled Myers diff with three context lines and conflict-marker parent labels normalized, independent of diff presentation settings. Preserving that patch preserves its id across a rebase or cherry-pick.
- **Base branch** — the artifact base branch of the **Artifact storage** convention: where pipelines start and merge. **Base** — the merge-base of the inspected tip with the branch its intent `starts-from`, else with the base branch, passed as `--base`.
...
- `patchId`; `patch`: the canonical diff bytes, base64-encoded.
- `files`: changed paths, each with `path`, `before`, and `after`. Repository-relative Git `path` bytes are base64-encoded. A side is null or `{mode, oid, bytes}`; blob `bytes` are base64, a gitlink has only mode and object id.
- `source`, in both file types: observed `commit`, ordered `parents`, and diff `base` (null for a root). These addresses are provenance.
- `digest`: SHA-256 of compact JSON containing `patchId`, `patch`, `files`, and `source`, in that order. Stored blob ids and the reconstructed patch must agree with the material.

Material preserves both sides of changed paths independently of the source commits. Failure to compute an automatic merge is an error, never an empty authored change.
...
A stamp follows the commit of what it stamps and is committed on top of it, on the branch the work landed on. A task report names the commits its task made and lands in a commit of its own after them.
...
| -------------------------------------- | ---------------------------------------------------------------------------- |
| `run-config.md`                        | none                                                                         |
| `changes/index.json`                    | occurrence pins                                                              |
| `changes/occurrences/<identity>.json`     | `material`: its patch material                                                |
| `changes/<patch-id>.json`                | none                                                                         |
