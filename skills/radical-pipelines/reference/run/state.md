...
- **Authored change** — what a commit introduces outside the pipelines folder: its diff against its parent (the empty tree for a root); for a two-parent merge, its diff against their automatic merge (`git merge-tree`). Identity is `git patch-id --verbatim` of the binary, rename-disabled Myers diff with three context lines and conflict-marker parent labels normalized. Preserving that patch preserves identity across a rebase or cherry-pick. Each occurrence of a patch is a package member.
