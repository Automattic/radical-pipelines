...
- **Wave** — one review of an artifact by every one of its lanes; numbered per artifact, shared by its lanes. A wave is valid on a package when every declared lane records `approved` and its `reviewed` pins equal the package it received in members and identities; a filtered lane receives its selected projection. The unfiltered package is validated when stamped and remains the historical certificate. A valid wave is current when its package is required now.
...
- **Episode** — an artifact's waves since its last valid wave; a counter `rp check` reports, never a gate.
