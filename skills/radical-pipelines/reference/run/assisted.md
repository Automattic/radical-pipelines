...
3. Design doc: after appending decisions, run `rp check`. Reconcile a stale spec first: re-synthesize it and its record in Synthesize mode with the decision delta under **Input changes**, even when its body stays unchanged; present the delta; have the owner re-approve it in a new review; commit and stamp the intent, spec, record, and review; then run `rp check`. Before synthesis, self-check: completeness of the areas above, clarity — two implementers would build the same thing — feasibility against the codebase by inspection, consistency between answers, explicit exclusions. Return to Q&A for any gap. Spec: surface the collected exclusions to the owner in one list and confirm them.
