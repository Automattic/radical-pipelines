...
6. On explicit approval: write the approval as the implicit lane's review — the artifact's next `<artifact>-review-<wave>.md`, `Verdict: approved`, `Brief: owner approval`, a verification log naming what the owner reviewed and anything the owner wants recorded. Commit the intent, artifact, record, and review together. `rp stamp` the intent and the record with `--mirror`, the artifact with its pins, and the review with `--reviewed` everything `state.md` says it names and `--mirror`; commit the stamps.
