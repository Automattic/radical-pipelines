...
5. On explicit approval: append the decisions to `intent.md`; `rp stamp` the intent with `--mirror` and the artifact with its pins; write the approval as a review — the next `<artifact>-review-owner-<n>.md`, `Verdict: approved`, `Charter: full scope`, a verification log naming what the owner reviewed — and `rp stamp` it with `--reviewed` both files, `--set lane=owner --set iteration=<n> --mirror`; commit everything.
