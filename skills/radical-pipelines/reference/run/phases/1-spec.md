...
- **Synthesize** additions: the phase folder's existing files; conditional **Lane inputs**; on re-synthesis, **Input changes** — every changed input's path and `git diff <spec.md's head> HEAD -- <input>` — and a **Correction** or **Task report** per unresolved challenge targeting `spec.md`.
- **Adjudicate** additions: the phase folder's existing files; one of **Review lanes** — every review file of the closed wave — **Correction**, or **Task report** — one per pending challenge, with the files its `origin` chain leads through; when adjudicating a consolidation, **Lane folders**.
