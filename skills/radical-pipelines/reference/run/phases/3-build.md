...
- Plan **Synthesize**: `1-spec/spec.md`, `2-design-doc/design-doc.md`, their approving reviews, the task reports so far (the done-set is work to build on, never redo), and the **Phase folder** files. **Input changes** on re-synthesis lists each changed input with `git diff <plan head> HEAD -- <path>`, then every unresolved challenge targeting the plan.
- Plan **Adjudicate**: the Synthesize materials plus `build-plan.md`, its **Tasks**, and `build-plan-research.md`, and one of **Review lanes** — every review in the complete closed wave (approving, rejecting, and unsatisfiable lanes); **Correction** — a challenge targeting the plan; or **Task report** — one failed report and its task file.
