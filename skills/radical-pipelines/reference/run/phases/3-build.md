...
- Worker: the **Task** file, its **Dependencies** (the task files it depends on), **Write your report to**; on a later attempt, **Your previous report**, the **Adjudication** when it adjudicated a failed one, and every **Review issue** attached to the task. The task's `Type` picks the profile: `tdd`, `edit`, `e2e`.
