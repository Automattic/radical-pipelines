...
- Plan **Synthesize**: `1-spec/spec.md`, `2-design-doc/design-doc.md`, `3-build/build-plan.md` with its tasks and reports, their approving reviews, the approving build review, the shipped code (the branch), the task reports so far, and the **Phase folder** files. **Input changes** on re-synthesis lists each changed input with `git diff <plan head> HEAD -- <path>`, then every unresolved challenge targeting the plan.
- Plan **Adjudicate**: the Synthesize materials plus `document-plan.md`, its **Tasks**, and `document-plan-research.md`, and one of **Review lanes** — every review in the complete closed wave (approving, rejecting, and unsatisfiable lanes); **Correction**; or **Task report** — one failed report and its task file.
