...
Derive the `<branch-base>` per the **Branch name base** convention and the `<pipeline-family-folder>` per the **Pipeline family folder** convention. Both are deterministic from the issue.
...
Inside the worktree, create the pipeline family folder containing:
...
**If** the body is already in the canonical format of `intent-format.md` and the issue has nothing else — no comments, references, links, or attachments: write it to `<pipeline-family-folder>/base/0-intent/intent.md` adding only the provenance header (per `intent-format.md`) and proceed to step 6.
...
- **Download screenshots or other assets** into `<pipeline-family-folder>/base/0-intent/` and reference them in `intent.md` by relative path.
