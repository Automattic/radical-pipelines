...
On an unmerged pipeline whose latest non-ejected run is complete, apply the amend qualification test (`amend-pipeline.md`) to the owner's request: when it passes, recommend the amend; when it fails — or the pipeline is merged — the option is not offered — and an owner who asks for an amend is told it does not qualify, and why.
