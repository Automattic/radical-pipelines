...
8. On **rejected**, build the next batch from the deduplicated list of task IDs the reviewer reported. When the rejection names work outside the plan's touch map, first relaunch a fresh `amend-lead` with the rejection file's path to adjudicate the map — extending `amend-plan.md` and its record with the sweep evidence, or ejecting when the map will not stay small and closed — and a fresh `amend-plan-reviewer` to re-approve the revised plan. Then go to step 6, with N incremented for the next rejection iteration.
