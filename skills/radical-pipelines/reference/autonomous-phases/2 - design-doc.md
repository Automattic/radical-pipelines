...
1. Run the lane flow in every lane; each lane writes its artifacts in its `lane-<K>` subfolder of the phase folder, so the lanes' paths are disjoint:
   ...
   - **Divergent mode** — run the lanes sequentially in the run branch's worktree, committing on the run branch. Assign each lane its **Lane mandate**: the first lane designs from the spec alone; each subsequent lane differs from the previous designs in at least one load-bearing decision; the last lane challenges an assumption all previous designs share. An owner-named **Lane angle** replaces that lane's default mandate.
