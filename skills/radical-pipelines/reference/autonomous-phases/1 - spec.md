...
1. Create one lane branch and worktree per lane (branch segment `1-spec-lane-<K>`, forked from the run branch) per the **Worktree root** convention, firing the `branch-created` and `worktree-created` lifecycle hooks for each.
...
3. When every lane is approved, merge each lane branch into the run branch, remove the lane worktrees, and delete the lane branches. Fire the `lanes-merged` lifecycle hook.
