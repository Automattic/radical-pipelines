...
4. After every doc-writer in the batch has committed, launch a fresh `doc-reviewer` with the list of task IDs in the batch, the base ref to diff against, and the rejection iteration number N (starting at 1, incremented per rejection — only used if this iteration ends in rejection). On rejection the reviewer writes `docs-review-N-rejected.md`; on approval it writes `docs-review-approved.md` (no number — the singleton terminator).
5. On **rejected**, build the next batch from the deduplicated list of task IDs the reviewer reported. Go to step 3, with N incremented for the next rejection iteration.
   - **Always-last re-dispatch rule:** any non-empty re-dispatch batch the orchestrator builds always includes the PR-description task, and that task always runs last in the batch. This is standing per-batch behavior that applies to every rejection iteration, not just the initial batch. Because the PR-description task is last in plan order with `Depends on` every other task, and step 3 runs the batch strictly sequentially in plan order, appending it last re-produces the PR description against the latest committed docs on every re-dispatch — so the description is never stale at approval, even when only an upstream doc task was flagged.
6. On **approved**, verify the phase 5 completion predicate per `pipeline-versioning.md` ("Per-phase completion"): all documentation changes, every `docs-review-N-rejected.md`, `docs-review-approved.md`, and `<artifacts-folder>/5-docs/pr-description.md` are committed on the pipeline branch.
