...
- `materials` — the pipeline-relative members selected from the review package in `../run/state.md` § Terms. The prompt declares that package; its references supply historical material. The lane declaration carries these paths; without it, the lane names the full package. **Diff**, **Brief**, **Help**, and **Write to** are always supplied and never filtered.
