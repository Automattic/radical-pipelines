...
For each dispatch, filter the blocks to those whose `agents` include the profile; a helper receives its requester's. Put their names and rules in the prompt template's **Guardrails** slot.
