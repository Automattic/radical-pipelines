...
Enumerate the family's branches and parse them with the branch grammar (`pipeline-versioning.md`); the latest run is the highest-`N` revision, or `base`. Reuse the run branch's worktree if it exists; otherwise recreate it from the branch per the **Worktree root** convention and fire the `worktree-created` lifecycle hook.
...
4. Fire the `phase-rolled-back` lifecycle hook.
