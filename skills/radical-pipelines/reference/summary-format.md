...
This describes a per-phase summary — a self-contained, human-friendly record of what its phase produced in the current run as a whole.
...
- **Cover the whole run.** Include every rejected iteration's surviving work, not only the final approved batch; the run diff you derived already spans this scope.
